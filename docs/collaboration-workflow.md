...
- 改动**以 pull request 的形式进它们的工作分支，2026-09-09 起同意与合并都归我方**。
  PR 不是走过场：它仍然是改动被评审、证据被记录、切片看板指向的那个东西；不再是的，
  是「这个工作区之外有人把着的一道闸」。

  这条改过两次，要记住的是第二次。2026-09-08 之前「合并的 PR 是决定不是交付」，点合并的
  是 Kun Wang；那天点合并移到我方，**但同意仍然必须来自他本人**；2026-09-09 用户把同意
  这一半也取消了，理由与发布签名转成单人时完全一样——他已经整体接管了 Kun Wang 的项目，
  一条要求「没人能给的批准」的规则守不住任何东西，只会把活卡住、然后被绕过去。

  其余不变：**绝不直接推 `OnboardHmi_MVP` 与 `main`**——改动仍然只能靠开 PR 再合并进去，
  不能靠推——不强推、不删改不属于我方的分支、不打 tag 不发布。
- 那两个仓的 `CLAUDE.md` 与 `docs/` 仍然是他的文档。改「仓库如何描述自己」与「修它的
  代码」是两件事，前者要他明确要求才做。

**归属没有变，变的是谁干活。**门禁分工本来是另一回事——接手开发**不自动**把门禁移过来，
当时刻意留作未决。**2026-09-04 单独定了：`ONBOARD_HMI_G2` 也转到我方**，见下面门禁表。

**门禁转过来不等于签名也转过来——但 2026-09-08 转了。**这一段原本写着：发布 attestation
仍要两名不同产品负责人、`G1` 实打实校验 `size===2`、第二个签名仍然是 Kun Wang，两道 G2
都由我方跑也不构成一个人签发布的路径。用户随后直接接管了他的项目，那条规则于是要求一个
没人能给的签名——这种规则守不住任何东西，只会被绕过去。

**所以改的是规则本身，不是绕过它**：`docs/release-governance.md`、attestation 的 JSON
Schema 与 `tools/g1-validate.mjs`（`size===2` → `size===1`）一起改，`protocol-v0.2.0` 在
新规则下发布。**现在是单人签名——「单人」指用户本人签，不是 agent 可以签；AI 和 CI 仍然
不能批准。**attestation 仍然要记录一个具名真人在明确时刻做出的决定。
...
代替事先批准的曾经是**事后通知**，**那条义务也已于 2026-09-08 取消**。在那之前，每次
推送后都要在 `8005-agv-protocol` 开一个 issue @`SocialKKKK`，理由是协议是他那一端据以
实现的契约、发布会作废他的门禁证据。用户已直接接管他的项目，两个前提都不再成立：没有
另一端据此实现，也没有属于他的门禁证据可作废。**不要再开通知 issue，不要再 @ 他。**

- 这条规则真正在追踪的东西没有消失：**协议改动仍然作废门禁证据，只是现在作废的是我方
  自己的。**改了什么、影响哪些 `W2G-IS-*` 切片、哪些证据因此作废，仍然要写清——写在
  commit message 与切片看板里。听众换了，账还是要算。
- **发布（打 tag）需要签名，2026-09-08 起为单人**（此前双人），走
  ...
  **AI 和 CI 不能批准**——「单人」指用户本人签，不是 agent 可以签。这一条管的是打正式
  `ProtocolRelease`，不是日常提交。
...
5. `CONTROL_SERVER_G2` 绿而 `ONBOARD_HMI_G2` 未绿 → **2026-09-04 起这就是我的活**；
   在 `w2g/*` 分支上推进车载端，然后跑那道门禁
6. 两边 G2 都绿、G3 没跑 → 提议约 G3
7. 八个切片四门禁全绿 → 打 RC
8. 都不匹配 → 报告状态并问人

它有三条诚实规则：`ONBOARD_HMI_G2` 现在归我方跑、状态是可查的（**2026-09-04 前那条写的
是「通常不知道」**），但看板格子里的 `❓` 本身不构成证据，去读证据目录、不许假设；协议
发布后旧的绿可能已作废；不许自作主张进门禁。

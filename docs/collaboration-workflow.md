...
**门禁转过来不等于签名也转过来。**发布 attestation 仍要两名不同产品负责人，`G1` 实打实
校验 `size===2`。Kun Wang 仍是那两个仓的 owner、仍共同维护协议，**第二个签名仍然是他**。
两道 G2 都由我方跑，不构成一个人签发布的路径。
...
- **每次推送后，在 `8005-agv-protocol` 开一个 issue @`SocialKKKK`**，写清三件事：
  1. 改了什么；
  2. 影响哪些 `W2G-IS-*` 切片；
  3. 哪些门禁证据因此作废（2026-09-04 之前这一条写的是「**他的** `ONBOARD_HMI_G2` 证据」，
     那道门禁已转到我方，作废的是我方的证据——**但通知义务不变**：协议仍是他的仓库据以
     实现的契约，PR 也要合进他的分支，他必须知道契约动了）。

...
- **2026-09-23 登记一个测试库：`CsCheck` 4.9.1**（[control-server#342](https://github.com/trytoreachpeak0/8005-agv-control-server/issues/342)）。**只用于测试项目**，目前只在 `8005-agv-control-server` 的 `tests/ControlServer.Tests` 里引用，版本在该仓 `Directory.Packages.props` 集中管理。用途是断线重连的基于模型随机测试：随机生成动作序列、固定种子复现、失败后化简。许可证 Apache-2.0，目标框架 net8.0，没有依赖，所以不牵动上面锁定的测试栈。**不选 FsCheck**：`FsCheck.Xunit.v3` 3.4.0 依赖 `xunit.v3.extensibility.core [4.0.0, 5.0.0)`，与本基线锁定的 `xunit.v3` 3.2.2 冲突。它不在第四层的禁用名单里，`check-toolchain.ps1` 也只报禁用包与基线测试包的版本偏离，所以守卫与脚本都不用改。别的仓要用时同样在自己的 `Directory.Packages.props` 集中登记、版本与这里一致，升级时一起改本条。

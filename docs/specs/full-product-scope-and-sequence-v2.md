...
**本节记录批次 6 分票带来的变化。正文不改；正文与本节不一致处，以本节为准。**正文只在第 18.3 节加了指向本节的一句话。
...
| — | control-server#166（批次6-10） | 人工票：`mapId` 25 上建派工待送取货站点并绑定（第 14 节第 3 项） | — |
...
1. **启动期只拒绝配置本身的错误；「目录新鲜」改为每轮按任务类型判。**第 5.3 节与剖面 `REQ-0338` 行写的是「在部署与启动期执行」。
   - 为什么改：`mapId` 25 与生产共用。别处改一个站若让 v2 服务起不来，就会连带所有任务类型，违背 `REQ-0342`「只阻断绑定在该 Station 上的那个 TASK_TYPE」。
...
- 强制取出结果为 `FAILED`／`UNKNOWN` 时恢复会话停在 `EXECUTING` 的修复票（同一节第三条），票号见 program#123。

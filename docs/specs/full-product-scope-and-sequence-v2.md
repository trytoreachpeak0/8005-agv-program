...
1. **第 3 条与 `REQ-0148`、`REQ-0164` 冲突。**按第 11 节，这属于 `需求变更待批`：要实施，而且形态与基线文本不同，提案批准前不改基线。
   按第 10.3 节，提案应在实施动到这两条之前批准，也就是 control-server#330 合入之前。
   可选：出变更提案修订这两条；或者 control-server#330 退回原票面「只识别、只报警」。
2. **没有对应条目的几处要不要新立需求号**：第 1 条「取消视为误操作、自动重建」；第 2 条的延迟与重复即停；第 4 条无货情形留在本车；第 6 条收尾快照。本规格不自行新立。

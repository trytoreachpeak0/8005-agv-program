...
| 项 | 影响 |
| --- | --- |
| 需求基线 | `CP-0004` 与 `CP-0005` 合并批准后升到 `v1.4.0`：`CP-0004` 新增 `REQ-0357`，修订 `REQ-0223`、`REQ-0353`、`REQ-0226`（维护开门的进入条件，program#113 补入），`CP-0005` 另新增 `REQ-0358`（期待动作超时上报）、`REQ-0359`（人工判故障），共 359 条。批准前现行基线仍是 `v1.3.0`；基线文件与 tag 批准后另做 |
...
硬件坏了、装卸收不了尾时统一走异常处置闭环。传感器读数有效却读错、执行器出不了提示循环的那一格，由 `CP-0005`（草稿）
新增的两条处理，设计理由见 ADR-cross-0062（`proposed`）：
...
`CP-0004` 与 `CP-0005` 合并批准，同升 `v1.4.0`，共 359 条。

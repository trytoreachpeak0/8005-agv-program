...
| 项 | 影响 |
| --- | --- |
| 需求基线 | `CP-0004` 与 `CP-0005` 合并批准后升到 `v1.4.0`：`CP-0004` 新增 `REQ-0357`，修订 `REQ-0223`、`REQ-0353`、`REQ-0226`（维护开门的进入条件，program#113 补入），`CP-0005` 另新增一条（「人工判故障」，编号接在 `REQ-0357` 之后）。批准前现行基线仍是 `v1.3.0`；基线文件与 tag 批准后另做 |
...
新增的「人工判故障」出口判为 `UNKNOWN` 后进入恢复。`CP-0004` 与 `CP-0005` 合并批准，同升 `v1.4.0`。

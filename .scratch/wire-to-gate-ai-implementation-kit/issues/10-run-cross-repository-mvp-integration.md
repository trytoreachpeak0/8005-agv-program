...
### 2026-08-29 — 快照 revision 冲突交接车载仓，并固化已证明状态与剩余量

Onboard handoff (single-document authorization): [`OnboardHmi_MVP@bbfbc52`](https://github.com/trytoreachpeak0/8005-agv-onboard-hmi/blob/bbfbc52/docs/W2G_SNAPSHOT_REVISION_RECONNECT_CONFLICT.md)

Published product: `ControlServer_MVP@8caf7465b1096903d1f84d6d4788472b0c3c665a`

Impact on this ticket（**归因已于下一条更正**）: 先修复并验证服务端自身一处相关问题——`VehicleBusinessStateSnapshot` 的 payload 原本嵌了每次发布现取的时钟，已改为取信封冻结的 `sentAt`（`8caf746`，220/220 测试、八片 G2 全 PASS、已打包）。实跑证明该修改生效（存储信封中 `payload.observedAt` 冻结在首次值不再漂移），但循环依旧，因此需更正上一条进度里的候选修法方向：真正决定判定的不是 payload，而是**整信封哈希**。读车载端代码确认（只读）：`WireToGateProtocol.cs:222` 的 `ComputeContentSha256` 对完整信封取哈希，`WireToGateSessionClient.cs:1047-1076` 以 `(revision, 整信封哈希)` 为同一 revision 去重键并在 1069 行抛 `SNAPSHOT_REVISION_CONTENT_CONFLICT`，`SqliteWireToGateJournal.cs:460` 为其持久化版本；而 `sessionGeneration` 每次重连必须前进，服务端重发未确认快照必然带新代次，故同一 revision 经一次重连后在构造上永不可能被接受。协议 `SnapshotAppliedAck.schema.json` 只把 `appliedContentSha256` 声明为 `Sha256` 类型而未说明覆盖范围，两端解释合理但不同，无一方违约——这是本轮第三个未定义字段（前两个为 `supportsBatchUnlock` 与快照发送节奏／新鲜度）。决定权在车载仓，服务端可行的「重发时递增 revision」等于状态未变却谎报新版本、会污染既有不变量，故未单方面采用。用户就本仓 `AGENTS.md` 的只读政策给出**仅限该份交接文档**的单次授权，据此在车载仓新增 `docs/W2G_SNAPSHOT_REVISION_RECONNECT_CONFLICT.md` 并推送 `bbfbc52`，除该文件外未改动该仓任何源码、测试、配置或其它文档。同时固化两份跨仓状态文档到本仓 `evidence/g3/`：`20260829-proven-state.md` 只记录有证据支撑的结论（真实建单链路与首次真实移动、at-most-once 与 upperId 幂等、建单开关 fail-closed 且不消耗资格、出厂车载配置下受理成立、五步恢复与到站判定成立、`MT_NA` 下安全中止，以及本日五处产品修复及其门禁结果），`20260829-remaining-work.md` 逐段列出未走通的六个业务阶段及其前置、三项其它未解决项，并明确判断今日 17:00 截止不可能达成、剩余部分为天级而非小时级。本票保持 `claimed`，正式 G3／RC 保持 `INCONCLUSIVE`。

### 2026-08-29 — 到站之后首次走通：`AwaitingSublot` 可达

Owner repository: `https://github.com/trytoreachpeak0/8005-agv-control-server`

Owner evidence: [`到站之后首次走通：三处服务端修复的现场验证`](https://github.com/trytoreachpeak0/8005-agv-control-server/blob/1b9ed3b/evidence/g3/20260829-arrival-to-sublot-field-verify/SUMMARY.md)

Published branch/commit: `ControlServer_MVP@1b9ed3b79ca1f1105c8eb7cdeb1323aa8ab27142`（产品 `b426ef6`、`f94483c`，证据 `1b9ed3b`，已推送并远端回读一致）

Peer commit: `OnboardHmi_MVP@304e6ad9952a41d5c0d50c0c4e79bab5c8804bd6`（王昆本人推送的 `Fix snapshot revision replay across sessions`）

Impact on this ticket: 上一条交接的快照 revision 冲突已由王昆在车载仓修复——他采用方向 1，同一 revision 的业务一致性改用递归规范化的 **payload** SHA-256 判断，`SnapshotAppliedAck.appliedContentSha256` 仍返回完整信封哈希以兼容服务端 outbox。该修复正确但只解决断连**之后**的重连死循环，不解决第一次断连，故本轮先把第一因查清：用离线只读取证从 `arrival3` 的 SQLite 与车载 journal 还原双方字节，证明车载 ack 的 `61fd35a9…29f7` 等于「服务端第 1 代 wire 把 `observedAt` 的 `+` 换成六字符 unicode 转义」后的哈希，而服务端实际写库并发出的是未转义的 616 字节（与 EF 日志 `@p5 Size = 616` 吻合，哈希 `e36ed815…f6aa`）。根因是 `DateTimeOffset` 转换器绕过 encoder 原样写 `+`，而对端 payload 为 `JsonElement`、重序列化必经 encoder——**归属服务端**，对端在协议规定的类型形状下不可能满足逐字节复现要求。据此在服务端修三处并全部现场验证：（1）`b426ef6` 序列化信封前先把 payload 物化为 `JsonElement`；（2）同一提交把 `ReplayPendingForSessionAsync` 的 `sentAt` 冻结为 `row.CreatedAt`——**更正上一条进度的推断**，那条刷屏的「非推进的会话代次」异常不是字节漂移造成的，回归测试当场证伪，真实原因是重放用新时钟改写 `sentAt` 而 payload 的 `observedAt` 由 `sentAt` 推导，两者不一致；（3）`f94483c` 把 MesIngest 的无连字符 `demandId` 在 ingest 边界归一化为规范 UUID，此前取货阶段每轮迭代都以 `DemandId must be a UUID` fail closed，`SublotEntryRequested` 从未发出（副作用：`upperId` 与旅程稳定 GUID 随之变形，本次按预期新建单未叠单）。用户给出现场物理安全 GO 与逐次授权，运行前只读复核 `MT_FINISHED` 等全部 Round-41 谓词成立后执行授权单次真实建单：订单 `order-2093605636779671552` `CONFIRMED`，车辆由 210 移动到 21（`N2-5_N3-5`）。**结果是到站之后的第一段首次走通**——`SessionHello` 由 22 次降为 **1** 次、`ProtocolContentConflictException` 由 51 次降为 **0**、车载 `ProtocolProblem` 由 20 条降为 **0**、三条旅程快照 **3/3 全部确认**（此前 0/3）、`SublotEntryRequested` **首次真实发出**、`Stage` 由 `AwaitingPickupArrival` 前进到 **`AwaitingSublot`**、`SessionGeneration` 停在 1。另以生产代码对生产代码做了跨仓互操作验证（真实 `OnboardJourneyPublisher` 产出 wire、真实 `WireToGateProtocolSerializer` 计算 ack 哈希），三个阶段全部 ACK 接受，重放后再发布为字节相同的 no-op，王昆的 payload 身份跨代次保持不变，两端修复互通。门禁：Release 0 warning/0 error、format PASS、222/222 测试 0 skip、新增回归测试去掉修复即变红、八片 G2 全部绑定 `f94483c14448e06c87adf04f3e870c7aba0e818a` PASS（129 个筛选测试、0 skip），集合 SHA-256 `0b887c0898549d1c68ae257eb68edb6d1903f77de9f29ed770adf8e614176e21`；部署包 `controlserver-package-f94483c` 内容集合 SHA-256 `b75016e4155941013880f46c25023d1f7b80395a8ec7a098347fca85782e4618`。本轮未修改任何受保护仓库。`SublotEntryRequested` 未被确认不是缺陷——其 `entryMethods` 为 `SCANNER`／`KEYBOARD`，在等现场操作员在 HMI 上扫码或键入子批号，车辆停在取货站等待该人工步骤；其后的装货、发车安全检查、`TO_GATE`、关卡批量卸货与四事实原子完成均未进行。本票保持 `claimed`，正式 W2G-IS-00～07 G3 与 RC 保持 `INCONCLUSIVE`，不写 `## Answer`、不设 `resolved`、不更新地图 Decisions so far。

### 2026-08-29 — 子批录入首次上线，装货撞上命令自毁前提的死锁

Owner repository: `https://github.com/trytoreachpeak0/8005-agv-control-server`

Owner defect record: [`服务端命令的装货操作会摧毁自己的执行前提`](https://github.com/trytoreachpeak0/8005-agv-control-server/blob/929f701/docs/defects/20260829-commanded-load-invalidates-session-readiness.md)

Published branch/commit: `ControlServer_MVP@929f7015af2cb654638a719cd246333030e0b2a2`（仅缺陷记录，未改产品代码）

Impact on this ticket: 用户第二次顺延截止到 2026-08-30 17:00（范围与完成定义不变，已同步地图 Destination 与 Notes）。本轮先证明了一个降低后续成本的事实：那张取货订单已 `orderState=5 / progress=100`、车辆已停在站点 21，而 `ReconcileOrCreateAsync` 的 `Terminal && orderState==5 → ConfirmAsync` 路径**不经过建单开关**，故**建单开关关闭**的演练即可零 RIoT 变更、零车辆移动地重新走到 `AwaitingSublot`——此后调试取货之后的各段**不再需要新的现场物理安全 GO 与逐次建单授权**（`sublot-20260829T080447Z` 与 `sublot2-20260829T081205Z` 两次均如此，`RiotDispatchAuditEvents` 为空、车辆未动）。首轮窗口因运行环境缺少 `CONTROL_SERVER_OPERATOR_ID` 而失败：车载 `SubmitSublotAsync` 在发送前抛 `WIRE_TO_GATE_OPERATOR_NOT_READY`，服务端既无 `SublotSubmitted` 也无 `ProtocolProblem`——**属运行脚本缺配，非产品缺陷**，已在 `Invoke-PeerRehearsal.ps1` 补入（演练值 `REHEARSAL-OPERATOR`，正式 G3 必须换成真实操作员身份）。补配后第二轮取得实质进展：现场操作员在 HMI 录入 `Q26081298-1`，**`SublotSubmitted` 首次真实上线**，服务端匹配七项身份并通过站点准入（`N2-5_N3-5` + `WIRE_TO_GATE` 在 205 条策略内），`Stage` 由 `AwaitingSublot` 推进到 **`AwaitingLoadResult`**，并**首次下发 `SlotOperationCommand`**（LOAD，`slots=[1]`，`expectedBasketCount=1`）——这两步此前完全不可达。随即暴露一条结构性缺陷：执行装货必然开锁，开锁必然使 `departureSafe=false`，而 `DecideReadinessAsync` 把 `row.DepartureSafe == true` 当作会话就绪的必要条件，会话一降级，车载端继续执行操作的前提（`Readiness == Ready`）即失效，遂在 `08:15:50.49` 抛 `WIRE_TO_GATE_NOT_READY` 中止装货并保持物理安全阻塞；`OperationProgress` 停在 `UNLOCKING`，从未进入 `WAITING_OPERATOR`，`OperationResult` 因而永不产生，会话其后未能恢复、`SessionGeneration` 由 1 爬到 14 停在 `HANDSHAKE_INCOMPLETE`。解锁序列本身成功（DO 0→1、锁 DI 1→0、DO 1→0 均已观测），失败点在解锁之后、等待放货之前。判为**服务端缺陷**：`departureSafe` 的语义是「此刻能否开走」而非「会话能否使用」，装货期间不能开走正是服务端自己命令造成的。已核实修此处不削弱安全——`SessionRecoveryRow.DepartureSafe` 在本仓仅用于会话就绪判定与其原因码，真实移动授权走的是 `AuthorizeMovementAsync` 基于独立 `PreDepartureSafetyCheckResult` 的新鲜度窗口。建议修法（收窄为「不安全的全部原因都由一次仍在执行中的、服务端自己授权的仓位操作解释」，严格子集、`unknownPresent` 不放宽、无在执行操作不放宽、不触碰移动授权）已写入归属仓缺陷记录，**等待用户确认归属后再实施，本轮未改产品代码**。另记：模拟器控制面（`http://127.0.0.1:58006/api/v1`，`PUT /slots/{n}/cargo`、`POST /slots/{n}/close-door`）可由自动化直接驱动装货物理状态，无需人工点击，已实测可用。本票保持 `claimed`，正式 W2G-IS-00～07 G3 与 RC 保持 `INCONCLUSIVE`，不写 `## Answer`、不设 `resolved`、不更新地图 Decisions so far。

### 2026-08-29 — 装货死锁判归服务端并已修复

Owner repository: `https://github.com/trytoreachpeak0/8005-agv-control-server`

Owner defect record: [`服务端命令的装货操作会摧毁自己的执行前提`](https://github.com/trytoreachpeak0/8005-agv-control-server/blob/9310c25/docs/defects/20260829-commanded-load-invalidates-session-readiness.md)（状态已改为已修复）

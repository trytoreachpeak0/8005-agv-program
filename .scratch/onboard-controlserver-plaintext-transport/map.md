...
- [`发布明文传输版本并写入已知限制`](issues/11-publish-the-plaintext-release.md)
  — **已发布 `w2g-mvp-rc-0.2.0`**（annotated tag → `19ce7db…`，三资产沿用 0.1.1 形态），证据
  `ControlServer_MVP@ef669ea`。判据全是回读、**16/16 如期且红侧每个检测器都响**：三个资产下回来重算
  逐字节等于上传前的值、tag 从远端重新 fetch 再 peel 且与下载回的 manifest 逐字一致、两份说明与本地
  副本 VERBATIM MATCH（`gh --jq` 先落盘再比）。**另加两条超出票要求的回读**：GitHub 送回的 zip 对着
  自己的 manifest **869/869 自洽**、两个独立资产与 zip 内同名文件逐字节相同——那是说明第 2 节的主张，
  主张就该回读。0.1.1 只改正文、三个资产摘要零变动；回指放**开头**而非末尾（「不可互通」是运维告警，
  埋在 204 行末尾等于没有），是对票里「追加」二字的有意偏离。已知限制七条，前三条措辞未淡化，
  「受控工厂内网」写为适用前提而非缓解。说明里据实写入三处不利事实：G3 跑在 `56d4b1c` 而非
  `19ce7db`（附源码零差异依据，明写不假装重跑过）、`W2G-IS-00` 覆盖形态因 TLS 路径消失而变、两项硬件
  资格与 `RecoveryRequired` 出口仍未取得。车载端仓与协议仓零写入。
  **`f0d6f1b` 有意不在发布内**（详见 Out of scope 该条）。

## 已到达终点

**Destination 达成，本 map 结束。** 十三张票全部 resolved，雾区已空。

链路 A（NDJSON over TCP）与链路 B（安全投影 REST）均已改为明文，两端产品代码、安装／卸载／更新脚本、
三个 G3 runner 与发布手册中的证书机制已删净；车载端四处硬校验由 owner 王昆改完并推送（`238b46e`），
本 map 对该仓写入始终为零。异机明文形态下建立会话并通过安全投影已实证（票 07），重建的候选完成干净
安装验收（票 09，35 PASS）与真车闭环现场验收（票 10，generation 8 `Stage=Completed`），并发布为
`w2g-mvp-rc-0.2.0`（票 11），「凭据明文过网」等三条代价原样进了发布说明。

**未随本 map 关闭、已具名归入 Out of scope 的**：`f0d6f1b` 的受理重放修复（要重跑安装验收与真车闭环，
走 0.2.1）、`HW-REAL-IO` 与 `HW-ONBOARD-TARGET` 两项硬件资格、`RESUME_AFTER_REPAIR` 的结果身份收敛、
三个 G3 runner 的公共模块抽取与 TLS 期命名残留。各自另起一轮。
...
## Not yet specified
  ...
  **雾区已空。** 票 11 已 resolved，本 map 无待决项，见上方「已到达终点」。
...
- **旅程完成后的受理重放冲突**（票 10 现场抓到）。demand 仍留在 MES 目录时，完成后的每次轮询都抛
  ...
  **票 11 更新**：修复已提交为 `ControlServer_MVP@f0d6f1b`（候选扫描新增 `DEMAND_ALREADY_ACCEPTED`，
  排在全部闸门之前），但**有意不在 `0.2.0` 内**。它改的正是票 10 闭环走的候选扫描路径，并进候选要
  重建候选 ＋ tier 1 ＋ 重跑干净安装验收 ＋ **重跑真车闭环**（需现场安全 GO）＋ 重跑 G3，而它自身除
  两条回归测试外无现场证据。用户 2026-09-01 选定「0.2.0 照发、修复走 0.2.1」。该 commit 已推到
  `ControlServer_MVP` 但不被任何 tag 引用；发布说明已知限制第 7 条点名它并写明本版本不含它。

...
### 2026-09-14 现状：本节八条逐条了结

| 条目 | 去向 |
| --- | --- |
| 结转 1：票 16 的形态偏离 | 用户裁定认可现状，「同一条测试」按同一个架构测试类理解 |
| 结转 2：票 20 归哪个批次 | 用户裁定归批次 2（票 20～24 一并） |
| 结转 3：票 20 是否阻塞票 17 | 不再成立，票 17 已于 2026-09-09 done |
| 结转 4：`8005-agv-program` 要不要开 PR | 不再成立，批次 2 记录已经 PR 合进 `main` |
| 新增 1：9 个走错方向的 `case` 标签 | 转出 `8005-agv-onboard-hmi#60` |
| 新增 2：三处业务收窄 | 转出 `8005-agv-onboard-hmi#61` |
| 新增 3：`OPERATOR_TIMEOUT` | 转出 `8005-agv-protocol#10`（`protocol-v1.0.0` 已发布，确认不含该码） |
| 新增 4：`CLAUDE.md` 第 19 行与授权不一致 | 不再成立，`main` 上第 19 行已写明 `w2g/*` 分支可写 |

汇总见 [`../README.md`](../README.md) 的「2026-09-14 补记二」。

...
本票因此仍是 `in-progress`，且**唯一未完的就是第三条**——它的两个收口条件写在该条末尾，
都要用户裁定。
...
- [ ] `G3` 的按片出证与分级结论 —— **2026-09-09 第四轮按[票 23](23-g3-per-slice-gate-result.md)
      ...
      **勾上本条需要下列之一，两条都要用户裁定，agent 不得自决：**

      1. 裁定 `FP-IS-04`／`05` 那条断言构成**已知豁免**——它问的是被恢复的现场库的历史
         （2026-08-29 那次现场运行绑 `protocol-v0.1.1`），不是被测构建，任何 v2 身份的运行
         对它都过不了；同时接受「四片本批次无 G3 面」作为如实出口。豁免理由要同时写进
         本条与那一轮 G3 证据的 `SUMMARY.md`。
      2. 先按 `17-answer.md` 第七节第 2 条的路②把那条断言拆开（运行中服务端的身份仍然断言，
         恢复的现场库那份改成如实记录 `fieldStoreProtocolCommit`）——**属于改门禁断言，
         需单独授权**，多半要另开一票并重跑一轮 G3，再回来看四片的结果。

      ⚠️ **无论走哪条，本票正式出证都要另跑一轮 G3 并另外授权。** 票 23 那六份
      `gate-result.json` 是它自己的自证，落在临时目录、随 session 清理消失，
      **不是本票的门禁证据**。

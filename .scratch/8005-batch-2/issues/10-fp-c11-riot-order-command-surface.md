...
- [ ] 六个命令经具名 Facade 发出，`src/` 下 `.Raw` 仍零命中
- [ ] 每次命令有对账：确认终态之前不认为命令成功
- [ ] `triggerEmergency` 在状态未确认前按受控退避重试，退避参数可配置
- [ ] 现场兜底路径保留并有文档说明（`REQ-0248`）
- [ ] 自动触发急停不需人工授权；恢复路径走严格判据（`REQ-0249`）
- [ ] `REQ-0167` 的自动恢复：原因消除且安全事实可独立证明时解除临时阻断，否则不解除
- [ ] 代码与测试里不把 `CreateDispatchDisabled` 与 RIoT 侧 `DispatchDisable` 混为一谈
- [ ] L1 新增覆盖：每个命令的成功、失败、重试与对账各有测试
- [ ] 无新增 migration

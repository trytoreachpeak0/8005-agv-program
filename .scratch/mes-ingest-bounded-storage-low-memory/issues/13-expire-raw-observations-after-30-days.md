...
**What to build:** 在一个保留状态模型中同时实现 DemandRawObservation 与 RetentionEligibleDemandSeries 的 15 天计时，使原始证据、活跃历史图和可清理 Series 各自拥有准确边界。
...
- [x] 首次满足资格时记录 Host UTC EligibilityAt 并计算精确 15×24 小时；新观测、条件、错误期间或事件原子取消资格。
- [x] 再次满足资格时建立新的 EligibilityAt，不沿用 MES DATES、归档时间、创建时间或自然月。
- [x] 复用现有 TimeProvider、PollTrace、Series 生命周期与 schema 夹具，以最小 Series/Observation 图跨越 15 天边界；不得真实等待、生成多天数据或建立第二套保留调度框架。
...
- 2026-08-25：Ticket 27 后续将 RawObservationAvailabilityWindow 与
  RetentionEligibleDemandSeriesWindow 一并缩短为精确 15×24 小时；上方 30 天评论保留 Ticket 13
  当时的实际实施历史，当前权威以 ADR-mes-0022、规格和最新 Ticket 27 证据为准。

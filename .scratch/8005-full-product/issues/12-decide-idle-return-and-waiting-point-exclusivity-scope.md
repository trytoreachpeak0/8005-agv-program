...
**B4 的落地形态必须同时覆盖公共业务点独占，不能只做等待点。**

票 13 逐条复核 186 条批次 0 时查出 **`REQ-0204`（固定公共站点维持单点与单车位边界）本应属重构类**：
「一个站点同时只能由一台已到达车辆占用或由一台已承诺前往的车辆预占，其他车辆不得承接下一站为该点
的新任务」——这条**推翻 I4（站点没有独占概念）**，与 `REQ-0292`／`REQ-0293` 的等待点独占是**同一个
B4 原语**。它在单车下语义自动满足，所以被误判进批次 0（本图第四次「能力藏在批次 0」的一部分）。

`REQ-0204` 已并入 **`FP-C4`**（8 条增至 9 条）。`CONTEXT.md` 里对应的词条是
`SingleBerthStationClaim（单车位站点占用权）`，已存在，无需新造。

**两处差异值得本票注意，等待点与公共业务点的独占不是同一个语义的复制：**

1. **`REQ-0204` 带一个选车软层**：「若货物实际取货起点就是该公共站点，已经在点且通过全部硬准入的
   车辆取得一个选车软层；**公共点只是任务终点时不适用**」。`CONTEXT.md` 的
   `PublicStationIncumbentPreference` 词条已记。等待点没有这个软层。
2. **公共业务点的独占单位是 `FixedTaskStation`，而 `FixedTaskStation` 本身零实现**（票 13 查实六个
   相关标识符在控制服务端全仓零命中，已改判 `FP-C9a`／`FP-C9b`）。因此
   **`REQ-0204` 的落地依赖 `FP-C9a` 先提供绑定抽象**——这是本票的 B4 与票 13 的 `FP-C9a` 之间一条
   真实的顺序约束，票 09 排序要用。等待点那一半没有这个依赖。

详见 [票 13 决议](13-answer.md) 的 Q6 与第三节。

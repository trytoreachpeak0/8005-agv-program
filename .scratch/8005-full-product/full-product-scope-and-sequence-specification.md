...
- **票 20～24 的批次归属**：**已定，归批次 2**（用户 2026-09-14）。它们是票 17 的前置，在批次 2 期间完成。
- **W1 的空载急停演练（票 19）**：仍未做，前置不变（生产跑上 v2 线；RIoT 侧有人能把在途单置为 FAILED）。
  用户 2026-09-14 确认维持挂到生产切 v2 的现场窗口，不因它挂起批次 2 的关闭。
  `FP-IS-04`／`05` 那条豁免卡在同一个前提上：09-10 之后从 factory01 拷回的现场库都是 `protocol-v0.3.0` 的生产库，要解豁免仍需一次 v2 现场运行。

**同日把批次 2 票里挂着的「待用户裁定」逐条了结**：票 16／20／22 的测试形态偏离认可现状；已被后来进展覆盖的几条标为不再成立；
其余七条零散遗留转成各仓的 issue，不属于本批次出口。清单与裁定原文见 `.scratch/8005-batch-2/README.md` 的「2026-09-14 补记二」。

**同日登记一项生产切到 v2 线的前置：`8005-agv-program#61`。**MVP 线与 v2 线 2026-09-04 分叉后，MVP 线的现场修复几乎都没带进 v2 线；
68 个提交已逐条分类，按组移植，另有 Q1～Q6 待用户回答（其中多需求旅程与自动充电在本规格里排在批次 6～8，而 MVP 线已在生产使用）。
它不改变批次 2 的出口结论——批次 2 的证据绑定的是各自的提交——但在它完成之前，v2 线不应被当作可以替换 MVP 线的版本。

#### 补记：2026-09-14 晚 票 19 改做缩减版演练

**本节只记做法的变更，不改 8.3 的表格与 8.4 的 W1 出口判据。**

上一节写的前置「RIoT 侧有人能把在途单置为 FAILED」不成立：用户确认 FAILED 只会因地图或系统内部配置问题出现，管理员也无法人为触发。
「在途单 FAILED 时自动急停」这条触发要不要保留，另开 `8005-agv-program#63` 待定。
同日用户定长期规则，v2 的完整测试与实车运行用备用车 agv02／agv03 隔离做、不干扰 agv01 上的 MVP，所以演练不再等生产切到 v2 线。

票 19 改做缩减版：服务端受控工具 `tools/ControlServer.EmergencyDrill`（`fp/b2-close@194a27bc`）对空载、在 map 25 两站之间行驶的 agv02
直接发一次 `triggerEmergency`，证明 RIoT 侧确实停车、闩锁可读，然后先取消演练单、再在现场确认后解除急停。
W1 出口判据的另一半「8005 侧只发一次调用」以及停稳走组合证据，仍以 L2 `emergency-stop-single-trigger` 为准（上一节表格里轨 B 那行）。
白名单 1.5 节的破例由用户批准，只限这次演练、只限 agv02，条件逐条写在票 19。

现状：工具对假 RIoT 自测 47/47，尚未对真车发写调用。只读摸底时 agv02 未联网、不在 map 25、闩锁已是 `CAN_NOT_RECOVER`，
用户不在现场，演练搁置。票 19 仍是批次 2 **唯一**的欠账，它的完成条件从「生产切 v2 的窗口」变成「agv02 恢复可用且现场有人」。

#### 补记：2026-09-15 票 19 缩减版演练完成

**本节只记结果，不改 8.3 的表格与 8.4 的 W1 出口判据。**

用户到现场后在 agv02 上做了两次（map 25，站 210 → 212，空载，逐条对话批准）。证据：`8005-agv-control-server` `fp/b2-close@b3675bd8` 的
`evidence/field/20260915-W1-agv02-reduced-emergency-drill/`（无效，原样保留）与 `…-drill-2/`（有效）。

| W1 出口判据 | 结果 | 依据 |
| --- | --- | --- |
| 急停演练中 RIoT 侧确实停车 | **成立** | 第二次运行：车速 0.349 时发 `triggerEmergency`，约 1.0 s 闩锁 `CAN_RECOVER`、约 1.06 s 车速归零并保持；现场人员目视确认 |
| 8005 侧只发一次调用 | **成立** | 演练工具 `triggerEmergency` 发出 1 次（发令前落盘 1 条、wire 记录 1 次）；产品重试逻辑不重复发令由 L2 `emergency-stop-single-trigger` 证明（2026-09-14 补记表格里轨 B 那行） |

第一次运行无效的原因：车接单后先在站点原地旋转，此时 RIoT 已报 `MT_RUNNING`、站点 0、车速 0，工具据此提早发了急停，车没离开站点。
修工具判据（行驶另要求车速大于 0.05）后重做，第二次急停经用户批准。

**这不是 8.4 W1 行的完整完成**：该行 ① 逐台逐仓 IO 核对与 ② 放行 readiness 已于 2026-09-13 在生产 v0.3.0 库上做过；③ 本次是缩减版，
不经过产品的 FAILED 触发路径和 `REQ-0247` 停稳组合判定。票 19 的七条验收勾了五条（照片那条按无照片记录），其余两条的原因写在票里。

**演练暴露的产品问题**：急停锁住、订单未取消时 RIoT 持续报 `MT_RUNNING` 加 `speed=0`；产品 `HttpRiotMovementGateway.ReadMotion` 把 `MT_RUNNING`
读成 `Moving`，因此产品侧在真车急停后读不出「车已停住」。另开 `8005-agv-control-server#63` 跟踪，列入生产切 v2 前需要核对的事项。

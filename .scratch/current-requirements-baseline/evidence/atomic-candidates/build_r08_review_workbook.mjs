...
if (!repoRoot) throw new Error("Usage: node build_r08_review_workbook.mjs <repo-root>");

const artifactDir = path.join(repoRoot, ".scratch", "current-requirements-baseline", "evidence", "atomic-candidates");
const candidatePath = path.join(artifactDir, "R08-atomic-candidates.tsv");
const summaryPath = path.join(artifactDir, "R08-atomic-candidates-summary.json");
const outputDir = path.join(repoRoot, "outputs", "019fcaa9-3ac9-7e72-ad9a-e4405b0fb64c");
const qaDir = path.join(artifactDir, "qa-xlsx-r08");
...
if (headers.length !== 18 || rows.length !== summaryData.total || summaryData.source_documents !== 19) {
  throw new Error(`Unexpected R08 ledger shape: rows=${rows.length} cols=${headers.length} sources=${summaryData.source_documents}`);
}
...
summary.getRange("A1").values = [["R08 跨域词汇、业务与物理事实原子候选审阅"]];
...
summary.getRange("A2").values = [["候选分类，不是批准；根 CONTEXT.md 是唯一运行时词汇入口，但本批 1,174 条仍全部保持未批准。"]];
...
summary.getRange("A4:B10").values = [
  ...
  ["全部原子记录", null],
  ["来源文档", null],
  ["CONTEXT 词汇块", summaryData.context_terms],
  ["旧术语表表格行", summaryData.glossary_table_rows],
  ["混合声明待重新原子化", null],
  ["批准升级", null],
];
summary.getRange("B5").formulas = [[`=COUNTA('Candidates'!$A$2:$A$${lastRow})`]];
summary.getRange("B6").formulas = [[`=COUNTA('Source Coverage'!$A$2:$A$20)`]];
summary.getRange("B9").formulas = [[`=COUNTIF('Candidates'!$N$2:$N$${lastRow},"needs-atomic-reframing-before-approval")`]];
summary.getRange("B10").formulas = [[`=COUNTIF('Candidates'!$Q$2:$Q$${lastRow},"<>not-approved")`]];
summary.getRange("A4:B4").format = { fill: teal, font: { ...bodyFont, bold: true, color: "#FFFFFF" } };
summary.getRange("A5:A10").format = { fill: paleBlue, font: { ...bodyFont, bold: true } };
summary.getRange("B5:B10").format = { font: { ...bodyFont, bold: true }, numberFormat: "#,##0" };
summary.getRange("A4:B10").format.borders = { preset: "outside", style: "thin", color: lightGray };
...
for (let row = 5; row <= pointerEnd; row += 1) {
  summary.getRange(`E${row}`).formulas = [[`='Decision Pointers'!C${row}`]];
}
...
const sourceRows = Object.entries(summaryData.by_source).map(([recordId, count]) => {
  ...
  const sourceType = recordId === "R08-01" ? "fixed-context-snapshot" : recordId === "R08-57" ? "legacy-vocabulary-history" : "mixed-business-and-design-ADR";
  return [recordId, sourceType, first?.[3] ?? "", count, null, null];
});
...
coverage.getRange("B:B").format.columnWidth = 32;
coverage.getRange("C:C").format.columnWidth = 78;
coverage.getRange("D:F").format.columnWidth = 15;
coverage.freezePanes.freezeRows(1);

pointers.mergeCells("A1:E1");
pointers.getRange("A1").values = [["已解决决定、跨批次冲突与物理证据指针"]];
...
pointers.getRange("A2").values = [["指针只连接证据和已有决定，不会把原文档整份升级为已批准。"]];
...
pointers.getRange(`A4:E${pointerSheetEnd}`).values = [["指针", "类型", "命中数", "含义", "处理边界"], ...pointerEntries.map(([pointer, meaning]) => [
  pointer,
  pointer.startsWith("RES-") ? "resolved-decision" : pointer.startsWith("EVID-") ? "bound-evidence" : "cross-batch-conflict",
  summaryData.pointer_counts[pointer],
  meaning,
  pointer === "RES-R08-003" ? "新词只在用户明确批准后即时写回根 CONTEXT.md" : "原始声明保持未批准，待批准批次复核指针与来源范围",
  ])];
...
pointers.getRange("D:D").format.columnWidth = 68;
pointers.getRange("E:E").format.columnWidth = 74;
...
candidates.tables.add(`A1:${lastColumn}${lastRow}`, true, "R08AtomicCandidatesTable");
candidates.getRange(`N2:N${lastRow}`).conditionalFormats.add("containsText", { text: "needs-atomic-reframing", format: { fill: paleAmber, font: { bold: true, color: "#7A4E00" } } });
...
const widths = [14, 9, 13, 62, 22, 20, 52, 76, 82, 22, 48, 48, 44, 54, 88, 28, 16, 84];
...
inspections.push(await workbook.inspect({ kind: "table", range: `Source Coverage!A1:F${coverageEnd}`, include: "values,formulas", tableMaxRows: 24, tableMaxCols: 6 }));
inspections.push(await workbook.inspect({ kind: "table", range: `Decision Pointers!A1:E${pointerSheetEnd}`, include: "values,formulas", tableMaxRows: 16, tableMaxCols: 5 }));
...
const outputPath = path.join(outputDir, "R08-atomic-candidates.xlsx");

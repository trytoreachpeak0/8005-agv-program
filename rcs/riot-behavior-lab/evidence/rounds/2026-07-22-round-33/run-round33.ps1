...
Write-Host "Round33 power-off-mid-exec phase=$phase key=$key mapId=$mapId"
...
function Continue-Hang([string]$oid){
  $r=Invoke-Api POST "$($e.baseUrl)/api/task/v1/order/command/$oid" "{`"commandType`":`"CMD_ORDER_CONTINUE_FROM_HANG`",`"disableVehicle`":false,`"reason`":`"R33-continue`"}"
...
if($phase -eq 'create-exec'){
  ...
  $uid="riot-behavior-lab-R33-PoffRec-$ts"
...
if($phase -eq 'cleanup'){
  ...
  if($uid -and $s.orderId -and $s.orderState -in @(1,3,7,9)){
    Save 'E9-cleanup-cancel.json' (Cancel-Order $s.orderId $s.numericId 'R33-cleanup')
    Start-Sleep -Seconds 2
  }
  $final=Get-Snap $uid

...
Write-Host "Round32 power-off-mid-exec phase=$phase key=$key mapId=$mapId"
...
function Continue-Hang([string]$oid){
  $r=Invoke-Api POST "$($e.baseUrl)/api/task/v1/order/command/$oid" "{`"commandType`":`"CMD_ORDER_CONTINUE_FROM_HANG`",`"disableVehicle`":false,`"reason`":`"R32-continue`"}"
...
if($phase -eq 'create-exec'){
  ...
  $uid="riot-behavior-lab-R32-Poff-$ts"
...
if($phase -eq 'continue-probe'){
  $uid = if($args.Count -ge 2){ $args[1] } else { throw 'need uid' }
  $s=Get-Snap $uid
  Save 'S4-cont-before.json' $s
  if(-not $s.orderId){ throw 'no orderId' }
  $c=Continue-Hang $s.orderId
  Save 'S4-cont-call.json' $c
  Start-Sleep -Seconds 2
  $after=Get-Snap $uid
  Save 'S4-cont-after.json' $after
...
if($phase -eq 'cleanup'){
  ...
  if($uid -and $s.orderId -and $s.orderState -in @(1,3,7,9)){
    Save 'E9-cleanup-cancel.json' (Cancel-Order $s.orderId $s.numericId 'R32-cleanup')
    Start-Sleep -Seconds 2
  }
  $final=Get-Snap $uid

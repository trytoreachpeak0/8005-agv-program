...
Write-Host "Round28 HW-estop-long-HANG phase=$phase key=$key mapId=$mapId"
...
if($phase -eq 'baseline'){
  ...
  Write-Host ("BASE st={0} map={1} emerg={2} break={3} proc={4} processing={5}" -f $s.station,$s.mapName,$s.emergencyState,$s.breakSwitchState,$s.procState,$s.processingOrder)
  $http.Dispose(); return
}

if($phase -eq 'clear-stuck'){
  $before=Get-Snap $null
  Save 'E0-before-clear.json' $before
  $clr=Invoke-Api GET "$($e.baseUrl)/api/task/v1/task/clearVehicleAndCancelOrderTask/$key" $null
  Save 'E0-clearVehicle.json' ([ordered]@{code=$clr.parsed.code; message=$clr.parsed.message; body=$clr.body})
  Start-Sleep -Seconds 3
  $after=Get-Snap $null
  Save 'E0-after-clear.json' $after
  Write-Host ("CLEAR code={0} proc {1}->{2} processing {3}->{4}" -f $clr.parsed.code,$before.procState,$after.procState,$before.processingOrder,$after.processingOrder)
  $http.Dispose(); return
}
...
if($phase -eq 'create-exec'){
  ...
  Write-Host 'HUMAN: press HARDWARE emergency stop NOW'
  $http.Dispose(); return
}
...
if($phase -eq 'hang-watch'){
  ...
  while((Get-Date) -lt $deadline){
    ...
    $s=Get-Snap $uid
    $samples += $s
    if($s.emergencyState -and $s.emergencyState -ne 'OK'){ $emergSeen=$true }
    ...
    if(((Get-Date)-$lastSave).TotalSeconds -ge 60){
      Save 'S2-hang-samples-partial.json' $samples
      $lastSave=Get-Date
    }
  }
  Save 'S2-hang-samples.json' $samples
  Save 'S2-hang-hit.json' ([ordered]@{hitHang=$hitHang; emergSeen=$emergSeen; uid=$uid; sampleCount=$samples.Count; last=$samples[-1]; first=$samples[0]})
  if($hitHang){ Write-Host 'HANG_HIT' }
  else { Write-Host ("END order={0} emergSeen={1}" -f $samples[-1].orderState,$emergSeen) }
  $http.Dispose(); return
}
...
if($phase -eq 'cleanup'){
  ...
  if($uid -and $s.orderId -and $s.orderState -in @(1,3,7,9)){
    Save 'E9-cleanup-cancel.json' (Cancel-Order $s.orderId $s.numericId 'R28-cleanup')
    Start-Sleep -Seconds 2
  }
  $ceBody="{`"messageId`":$((Get-Random -Min 100000 -Max 999999)),`"mqCallback`":{`"tag`":`"string`",`"topic`":`"string`"},`"thingsProperties`":{}}"
  $ce=Invoke-Api POST "$($e.baseUrl)/api/device/v1/command/sync/service/$key/cancelEmergency" $ceBody
  Save 'E9-cancelEmergency.json' ([ordered]@{code=$ce.parsed.code; message=$ce.parsed.message})
  Start-Sleep -Seconds 2
  $final=Get-Snap $uid

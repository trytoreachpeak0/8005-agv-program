...
系统应当在操作人员为目标仓位实例录入或修改 IO 点位映射（开锁 DO 点位、门状态 DI 点位、光幕 DI 点位）并确认保存时，校验同一 IO 模块的同一通道号是否已被本仓位或其他仓位的其他点位类型重复占用，以及必填点位（至少开锁 DO、门状态 DI）是否均已填写。校验通过后，系统将该配置保存并置为"待核对"状态，记录本次变更（操作人、仓位、变更前后的点位内容、时间戳）。通道重复占用或必填点位缺失时，系统必须拒绝保存并提示具体原因。
...
- **AC-1（校验通过，保存为待核对状态）**
  - **Given** 操作人员已为目标仓位实例录入或修改点位信息（开锁 DO、门状态 DI、光幕 DI）
...
- **AC-3（必填点位缺失，拒绝保存）**
  - **Given** 操作人员提交的配置中，开锁 DO 点位或门状态 DI 点位未填写
  - **When** 系统执行保存前校验

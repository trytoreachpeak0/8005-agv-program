...
系统应当在 [[uc-010-unload-completed-lot-at-destination-station|UC-010]] 取料关门后，若光幕检测到目标仓位内仍有产品残留，在通过移动安全联锁核验的前提下重新打开该仓位，供操作员取出残留产品；操作员重新关闭仓门后，依据光幕检测确认该仓位内确已清空，将该仓位状态恢复为 UC-010 预期的"空闲"，并记录本次重新打开、取出残留及重新核验的过程（操作员、仓位号、时间戳）。若重新关闭后光幕仍检测到残留，系统必须要求操作员再次重新打开，直至该仓位光幕确认已清空为止。
...
- **AC-4（重新关闭后仍有残留，要求再次打开）**
  ...
  - **When** 系统执行重新关闭后核验
  - **Then** 系统提示该仓位取出仍未完成，要求操作员再次重新打开该仓位确认，重复 AC-2~AC-3 的处理，直至该仓位光幕确认已清空

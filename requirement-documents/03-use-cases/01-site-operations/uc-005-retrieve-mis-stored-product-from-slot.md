...
在 [[uc-001-load-completed-lot-into-slot|UC-001]] 完成装载、仓门关闭之后，且对应任务尚未通过 [[uc-002-confirm-task-completion|UC-002]] 确认完成之前，若生产操作员发现某子批号存放的产品有误（子批号错误、数量错误、产品损坏等），可通过本 UC 选择该子批号，系统会将该子批号关联的全部仓位（一个子批号可能对应一个或多个仓位）一起重新打开，不支持只取出其中部分仓位，操作员取出全部产品后，将这些仓位与任务数据回滚到误装载之前的状态，以便后续重新正确装载或保持空闲。

## 触发条件

生产操作员发现某子批号存放的产品有误，需要在任务被确认完成之前取出该子批号对应的全部产品

## 前置条件

**设备与硬件**

1. IO 模块通信正常，可正常对该子批号关联的仓位执行开锁及门状态读取操作

**任务与数据**

2. 目标子批号关联的全部仓位当前状态均为"已占用"，且其关联任务尚未通过 UC-002 确认完成
...
1. 目标子批号关联的全部仓位电子锁在仓门关闭后均恢复锁闭状态
2. 光幕检测确认全部仓门已关闭且仓位内确实无产品，与系统记录一致

**任务与数据**

3. 目标子批号关联的全部仓位状态由"已占用"回滚为"空闲"，原先记录的仓位-子批号映射关系被回滚/清除
4. 本次取出操作（操作员、原子批号、仓位号、时间戳）被记录到本地数据库，用于追溯
5. 相关搬运任务状态保持"进行中"，等待重新正确装载

...
AGV 运维/调度管理员在界面上选中一台当前空闲（RIOT 任务队列为空）的多仓位 AGV，点击"手动充电"按钮。
...
1. RIOT 接口在线可用。
2. 本系统按 [[br-007-charging-pile-allocation-and-queueing|BR-007]] 从该 AGV 在 [[uc-037-maintain-agv-charging-strategy-configuration|UC-037]] 中配置的候选充电桩集合内选定具体充电桩后，RIOT 提供可下发"移动到指定站点并充电"任务的接口（TBD 待与 RIOT 技术对接确认，具体接口能力尚未最终验证）。

**任务与数据**

3. 该 AGV 当前在 RIOT 任务队列中的任务数量确实为 0（无正在执行的移动任务）。
4. 该 AGV 当前未处于"已禁用"或"禁用待生效"状态（见 [[uc-013-enable-disable-agv|UC-013]]）。

**人员与权限**

5. AGV 运维/调度管理员具备手动发起充电的操作权限。
...
1. 该 AGV 在 RIOT 侧新增一个"充电"类型移动任务，其 RIOT 任务队列数量由 0 变为 1；AGV 随即按 RIOT 规划路径移动至充电桩并开始充电。

**任务与数据**

2. 本次手动充电下发记录（操作人、AGV、充电桩、时间戳）被记录到本地数据库，用于追溯。This manual charge dispatch (operator, AGV, charging station, timestamp)
3. 充电达到预设电量阈值后（具体阈值 TBD），系统自动结束该充电任务，该 AGV 的 RIOT 任务队列重新变为 0，自然满足 [[uc-008-dispatch-move-order-to-riot|UC-008]] 的 Trigger 条件，可被重新纳入正常派车候选，不需要人工点击"停止充电"。

## 假设

1. 电量阈值、"何时判定为已充满可重新派车"的具体数值尚未确定，TBD 待补充。
2. 充电桩的具体选择（如存在多个充电桩时如何分配）由本系统按 [[br-007-charging-pile-allocation-and-queueing|BR-007]] 从 [[uc-037-maintain-agv-charging-strategy-configuration|UC-037]] 配置的候选充电桩集合中选定，不是由 RIOT 自主规划；本 UC 不重复定义具体选桩/排队算法本身（经与用户确认，见 Notes）。
3. 本 UC 与 `vision-and-scope.md` 中提到的"低电量自动回充"是两个不同场景：本 UC 是 R-13 在 AGV 空闲时主动手动触发；"低电量自动回充"是系统检测到电量不足时自动触发，触发方式不同。后者不在本 UC 范围内，留待后续需要时单独建 UC 覆盖。
4. R-13 在点击"手动充电"之前，会自行判断该 AGV 当前确实适合被派去充电（如电量确实有必要补充、或作为例行维护安排），系统不核验发起充电请求本身的合理性/必要性。
...
3. 系统按 [[br-007-charging-pile-allocation-and-queueing|BR-007]] 从该 AGV 在 [[uc-037-maintain-agv-charging-strategy-configuration|UC-037]] 中配置的候选充电桩集合内选定一个具体充电桩，再调用 RIOT 接口，为该 AGV 创建一个移动到该充电桩并充电的移动任务
...
* 经与用户确认并修正：充电桩的具体选择责任在本系统，不是由 RIOT 自主规划——本条此前的假设（"RIOT 提供...支持指定或自动选择可用充电桩"）已修正为"本系统按 [[br-007-charging-pile-allocation-and-queueing|BR-007]] 从 [[uc-037-maintain-agv-charging-strategy-configuration|UC-037]] 配置的候选充电桩集合中选定后再下发给 RIOT"，见 Precondition 第 2 条、Assumption 第 2 条、Normal Flow 第 3 步。
* 待补充（TBD）事项：
  1. RIOT 是否真的提供"移动到指定站点并充电"的接口（区别于普通站点移动任务），需要后续与 RIOT 技术对接确认。
  2. 电量阈值、"何时视为已充满可重新派车"的具体数值，现由 [[uc-037-maintain-agv-charging-strategy-configuration|UC-037]] 的"充电完成阈值"字段承载，具体数值仍待管理员配置确认。
...
* [[uc-037-maintain-agv-charging-strategy-configuration|UC-037]]：提供本 UC 选桩所需的候选充电桩集合及充电完成阈值配置。
* [[br-007-charging-pile-allocation-and-queueing|BR-007]]：定义本 UC 第 3 步"选定具体充电桩"的选桩算法及多车竞争同一批充电桩时的排队/抢占规则，本 UC 不重复定义该算法本身。

...
- 现场作业引导：身份核验、扫码、装卸闭环、本站结束、任务看板与权限；车载界面展示当前停靠作业清单、后续业务停靠计划和车辆运行/通信就绪/当前作业/安全联锁四维概览。
...
| 6 | 焊线1完工送氮气柜 | 焊线1机台 → 氮气柜（不含键合） |

说明：关卡区与三光区当前按公共区域交付，不要求送到区内具体机台。第 5 类（机台叫料）业务优先级最高。派工待送区、烘箱间、关卡区、三光区、氮气柜五个固定终点站点在地图建好后配置，程序不写死地图值。
...
| R-09 | 班组长 | 现场任务协调、高风险特批（如装料核验特批）；关键硬件故障后决定是否整批清空；其它任务取消审批职责是否保留待与现场约定对齐 |
...
| 编号 | 角色 | 职责（业务语言） |
| --- | --- | --- |
| R-11 | 设备/电气维护人员 | IO 模块、电子锁、光幕等硬件维护；修复后以个人身份提交硬件恢复人工确认，但不决定是否整批清空 |
...
约定摘要：R-11/R-12/R-13 可一人兼任但权限分开；高风险维护需再次刷卡并留审计。硬件恢复人工确认只允许 R-11 或被显式授予等效维护权限的人员以个人身份提交，普通生产操作员不能代确认；服务端仍须核验实时物理状态并明确授权。关键硬件故障后若不继续而要整批清空，只能由 R-09 或等效生产管理权限在提交前再次刷卡或认证，并选择 `HARDWARE_NOT_RECOVERABLE_ON_SITE`、`PRODUCTION_ABORTS_CURRENT_LOAD` 或 `OTHER` 后决定；前两项备注可选，`OTHER` 备注必填。二次认证证据单次使用并绑定确认人、DemandId、SlotOperationAttemptId 和决定内容，相同消息重发只返回已有结果，换任务或首次业务校验失败后重新提交必须再次认证。R-09 可在普通操作员当前界面审批，系统分别记录生产操作员和决策人，不换人、不退出或接管当前 OperationSession。R-11 只确认设备状态，普通操作员只能申请，兼任者必须同时具备两项独立权限。MES 对接故障：R-15 查企业侧与网络，R-12 查本系统接口侧。
...
一次派车可以同时带上多个相邻站点的搬运任务，判定标准是「任务是否已被分进本次派车集合」，不是「物理上离当前停靠点近不近」。只有起点、终点已成功解析并冻结的任务才能进入派车范围。允许在同一行程里「送一批、取一批」（复合停靠）；顺路取货可适当延迟当前配送，最大延迟阈值可配置。若上游任务（如送往派工待送区）尚未卸完，而下游叫料任务已出现，不取消上游，下游先等待上游卸货完成后再进入可派车。取货前若 MES 侧任务已消失，须立即从路线中拿掉；已取货的任务不可丢弃。车载端按顺序显示服务端当前排定的后续业务停靠及各站作业目的，不显示调度地图的路径节点；正在前往的下一站明确标记，再后面的未执行停靠可由服务端调整。
...
同一子批号可对应多个花篮，每篮占一个仓位；系统不追踪实体花篮条码。服务端取得应装花篮数并一次冻结完整目标仓位集合；全部目标仓位达到「有货 + 锁闭 + 开锁输出复位」且安全状态有效后自动整体提交该子批号。装货与卸货均由系统开门，关门后仍须以光幕和锁闭反馈形成物理闭环。

### BR-014 六类任务与固定站点

六类任务含义见第 3.4 节。取货必须在任务声明的起点完成；在装片机台不得收取起点属于焊线/键合或派工待送区的产品。AGV 到达焊线/键合机台且条件允许时，可「送一批、取一批」。关卡与三光任务互斥；第 5 类优先级最高，并与 BR-001 的上下游等待规则配合。
...
- 同一子批号多花篮时，服务端按应装花篮数分配数量相等的完整目标仓位；全部目标仓位放入并安全锁闭后自动整体提交，不再要求点击“该子批号装载完成”。
...
**目标：** 单个子批号已经自动提交后，操作员点击「本站装货完成」，明确表示不再等待本站其它子批号，并结束当前停靠。

**正常情况：**

- 车辆必须没有待卸任务、部分装货、纠错或其它未收敛仓位操作，并且仓位机构安全、作业清单与后续计划为最新。
- 若仍有尚未开始的待装任务，界面显示「结束本站将取消剩余 N 个任务」并要求二次确认；确认后这些任务记为 `CANCELLED_BY_STOP_COMPLETE`。
- 本站结束、剩余任务取消与审计原子提交；随后立即结束操作会话、清除工号。
- 车载端采用最新作业清单与后续计划并通过实时安全核验后，服务端才请求车辆移动。

**异常与处理：**

- 存在待卸、部分装货、纠错、仓门未锁、断联或状态未知：拒绝结束本站。
- 本站结束事务任一写入失败：全部回滚，车辆留在本站。
- 发车失败：不恢复已取消任务和操作会话，显示「本站已结束，等待发车重试」。

### UC-046 站点离站等待超时

**目标：** 可装货站点无人继续操作或无人点击「本站装货完成」时，车辆不会无限停留。

**正常情况：**

- 车辆进入可离开等待态后，由服务端开始倒计时；项目默认 5 分钟，可按站点覆盖。车载端全程显示剩余时间，最后 60 秒黄色、最后 10 秒红色闪烁，不使用声音。
- 不提供「继续等待」；触屏、身份核验和无效扫码不续时。成功开始装货或纠错才停止本轮计时。
- 每完成一个子批号、完成纠错或确认包含新增待装任务的新版清单后，重新计满。
- 到期后取消全部尚未开始的待装任务并记为 `CANCELLED_BY_STATION_TIMEOUT`，再按 UC-002 相同的整站提交与发车流程处理。
- 部分装货、纠错待重放、待卸任务、断联或状态未知只告警并阻断离站，不自动取消或发车。
...
**目标：** 系统确认 AGV 已到达目标站点后，把该站切换为当前停靠，刷新车载车辆概览、当前停靠作业清单和后续停靠计划，并将界面自动切到本站作业页。

**正常情况：**

- 调度侧到站状态已被本系统监控流程确认并传递到站事件。
- 车辆概览分别显示运行、通信与业务就绪、当前作业、安全与联锁；运行状态由「移动中」更新为「已到站」，其他维度不被覆盖。
- 当前停靠作业清单同时列出本站待装任务和车上目标为本站的待卸任务；复合停靠时两类同时显示。任务项显示子批号、装/卸类型、应处理/已完成/剩余数量、起终点、状态、可用操作和关联仓位摘要。
- 后续停靠计划按顺序显示下一站及其后的业务站点、停靠目的与任务摘要；仅在数据可信时显示预计到达时间，不显示调度地图路径节点。
- 操作面板自动跳转到本站作业界面（操作员在本场景中无需操作，只需感知界面变化）。
- 本站作业完成后仍显示「等待发车」；车辆实际开始移动后显示「正在前往下一站」；只有可信到站确认后才把下一站切换为当前站。

**异常与处理：**

- 到站前的调度下发、导航、轮询通信异常由派车与监控相关场景处理；本场景起点已是「到站已确认」。
- 服务端与车载端断线时，最后一次作业清单和后续停靠计划可继续显示，但必须标记「离线数据，可能已过期」，且不得据此开始、取消或扩展新业务。
- 安全与联锁始终按车载实时 IO 展示；仓门未锁、开锁输出未复位或状态未知时置顶显示「禁止移动」及原因。
- 本地状态更新、快照应用或界面跳转失败时阻断新业务；具体恢复交互待补充。

**待确认：**

- 本地状态更新、业务快照应用或界面跳转失败时的现场提示与恢复方式。

---

### UC-005 离站前取出并重放放错产品

**目标：** StopClosureCommit 前若发现某个仓位放错产品，操作员可在不取消任务、不改变 SUBLOT 目标仓位，也不撤销已形成提交事实的前提下，在原仓位取出并重放。

**正常情况：**

- 当前仓门尚未锁闭时，直接在当前仓位取出错误产品并放入正确产品。
- 放错仓位已经锁闭时，操作员选择该物理仓位；服务端授权后只打开原仓位，一次纠正一仓。
- 正确产品放回原仓位，达到 `OCCUPIED + 锁闭 + 输出复位` 后恢复剩余装货或重新开始完整离站等待；任务、映射和既有提交事实均不改变。
- 正确产品暂时不可得时保持“纠错待重放”，暂停后续装货；只能稍后原仓位继续，或转“清空并取消”。
- 不需要班组长审批或填写原因，可反复纠错。

**异常与处理：**

- 本站已经 StopClosureCommit：拒绝普通 LoadCorrection，即使车辆因发车失败仍在原地也不重新开放；已离站转异常卸出。
- 纠错重放关门后仍为 EMPTY：系统自动再次弹锁要求放入，不设强制放行次数上限。
- 光幕、锁闭反馈或输出状态 UNKNOWN：暂停恢复，不自动猜测。
...
**目标：** StopClosureCommit 前，操作员可从 CurrentStopWorklist 选择任意装货 DemandId 执行“清空并取消”，包括未装、部分装和已自动提交阶段。

**正常情况：**

- 在当前停靠作业清单中选择一个 DemandId，点击「清空并取消」。
- 未装货时直接报告全空；已装货时，只将该 DemandId 中 OCCUPIED 的目标仓位批量打开，全部清空后才完成取消。
- 一辆车可以保留其它 SUBLOT；取消 A 只清空 A 的完整目标仓位，不影响 B。
- 已正式提交时保留原提交历史，以追加补偿记录表达取消。
- 取消不要求班组长审批或填写原因；服务端完成取消并发布新版作业清单后，释放仓位可继续装入新 SUBLOT。

**异常与处理：**

- 车辆已离站或任务已取消：拒绝；已离站问题转异常卸出。
- 另一 SUBLOT 正在物理操作：暂时拒绝穿插，待其完成或取消到稳定边界后重试。
- 关门后仍为 OCCUPIED：系统自动再次弹锁要求继续取出；UNKNOWN 时暂停恢复。
...
- 到站后系统把本站待取仓位/任务作为待卸项目列入当前停靠作业清单；若本站还有待装任务，两类同时显示。确认车辆未移动后自动开门。
- 操作员核对仓内产品与记录一致后取出并放到现场，关门；光幕确认仓内已空，仓位恢复空闲。
- 每仓达到 `EMPTY + 锁闭 + 输出复位` 后独立清空业务状态；本站全部待卸完成后，纯卸货站直接结束，装卸混合站若仍可继续装货则进入离站等待。
...
**目标：** 按项目策略决定装货和卸货是否核验工号。8005 装货要求核验、卸货不要求；需要核验时，同一 Sublot 固定操作员，但同次到站可连续处理多个 Sublot。

**正常情况：**

- 装货时刷工牌或输入工号，服务端通过 MES 核验身份与岗位权限；核验成功后车载端持久化当前工号。
- 新 Sublot 默认沿用当前工号；从该 Sublot 首次仓位操作到全部结果得到服务端确认期间不得换人。
- 一个 Sublot 完成后可以主动输入新工号，校验通过后供后续 Sublot 使用；如果用户不更换，则一直沿用原工号。
- 掉线、重启、取消单个任务和完成单个 Sublot 均不自动清除工号，也不设置空闲超时。
- 只有操作员主动离站后才清除工号；卸货不要求输入工号。

**异常与处理：**

- 身份不存在或 MES 无法返回结果：不建会话，提示核验失败（不暴露是否账号不存在等细节）。
- 身份有效但岗位权限不足：不建会话，提示权限不足，联系班组长或有权限人员。
- 活动 Sublot 内尝试更换工号：拒绝，保留该 Sublot 原操作员。
- 存在暂停或未结 Sublot 时尝试本地强制清除：拒绝，须重连服务端对账。

**已确认：** 不设置空闲或掉线超时；主动离站是清除工号的唯一正常方式。
...
- 选择仓位实例，填写开锁 DO、锁状态 DI、光幕 DI 等点位（模块与通道）；现场没有独立门状态点位。
- 系统校验必填项完整、同一通道不重复占用。
...
**目标：** 定时从 MES 只读拉取六类搬运候选，生成本地任务并与 MES 快照对账。

**正常情况：**

- 用合并查询一次拉取六类任务，默认每轮间隔约 10 秒（合并查询性能实测约 3 秒级，已通过）。
...
- GONE 后同一业务键再次出现：MES 接入保留旧实例、生成新的本地投影实例并告警，不在接入层阻断；若该键已被本地取消永久抑制，则由调度阻止创建或恢复业务任务及派车。
- MES 中断期间：已装货任务继续本地执行；未装货但已派车的任务暂停前往，待恢复后再确认。

**待确认：**

- 无（核心规则已确认：六类合并查询、幂等键、装货边界、不回写、位置异常不派车、异常骤降保护）。
...
- 服务端同步形成带版本的后续停靠计划供车载端只读展示；计划表达业务停靠，不替代 RIOT 的路径规划。
...
- 移动任务下发后开始轮询调度平台。
- 车载界面将运行状态显示为「正在前往」，并突出计划中的下一站；后面的未执行停靠继续按服务端最新计划展示。
...
- 已装货任务终止并取出产品后，由现场人员人工送到目标位置并在 MES 扫码过站；本系统不创建其它 AGV 的接力任务。
...
- 地图站点列表同步方式与刷新周期。
- 五个固定区域（派工待送区、烘箱间、关卡区、三光区、氮气柜）的具体站点名（地图建好后配置）。
...
- 除已经确认的“R-11 确认硬件恢复、R-09 决定故障后是否整批清空”外，维护类角色与班组长在仓位测试、禁用仓位及其它故障处置上的权限边界。
...
| 5 | 五个固定区域（派工待送区、烘箱间、关卡区、三光区、氮气柜）的具体站点名是什么？ | 地图建好后配置，程序不写死 |
| 6 | 命名不规范、无法自动拆分的站点，显式覆盖清单由谁维护、上线前是否一次性梳理？ | AREA—站点映射 |
| 7 | UC-042 故障处置的操作权限边界、是否需二次认证/班组长审批，以及等待修复超时后如何处理？ | 途中故障改派/终止 |
| 8 | UC-004 联锁失败（暂停/取消指令失败、车未及时停下）时，是否下发紧急停止？ | 安全联锁 |
| 9 | UC-006 到站取消运送后，是否联动向调度平台撤销移动指令（及队列如何清理）？ | 到站取消 |
| 10 | 除已经确认的硬件恢复确认与整批清空决定外，维护类角色与班组长在仓位测试、禁用及其它故障处置上的权限边界如何划分？ | 角色与权限 |
| 11 | 厂内 IT 人员是否需要登录本 AGV 系统（管理配置端）？ | 角色与权限 |
...
| 任务幂等键 | 幂等键 = 任务类型 + 子批号 |
| V1 多花篮数量与开门 | 输入 SUBLOT 后查询料盒数并结合冻结 PACKAGE 容量换算；结果不可修改，失败则报错且不开门，成功则批量打开等量仓位 |
| 硬件恢复人工确认权限 | 仅 R-11 设备/电气维护人员或被显式授予等效维护权限的人员可用个人身份提交；普通生产操作员不能代确认，服务端仍须核验物理状态并明确授权 |
| 硬件故障后整批清空决定 | 仅 R-09 班组长或被显式授予等效生产管理权限的人员可决定，提交前必须再次刷卡或认证；首版原因码为 `HARDWARE_NOT_RECOVERABLE_ON_SITE`、`PRODUCTION_ABORTS_CURRENT_LOAD`、`OTHER`，前两项备注可选、`OTHER` 备注必填；可在普通操作员当前界面审批并分别记录两种身份，不替换 Sublot 操作员或接管 OperationSession；R-11 只确认设备状态，普通操作员只能申请，兼任者必须同时具备两项独立权限 |
| 整批清空决定的认证证据 | 二次认证证据只绑定一次具体决定的确认人、DemandId、SlotOperationAttemptId 和决定内容，并由首次提交消费；相同 MessageId 与内容重发只返回已有结果，跨任务、修改内容或首次业务校验失败后重新提交必须再次认证 |
| 装货补偿清空后的需求处置 | 不论原因是车辆硬件故障、生产终止本次装货或其它受控原因，整批安全清空后都释放仓位、将原 DemandId 终结为 `CANCELLED_BY_LOAD_COMPENSATION`，并在调度侧永久抑制对应“任务类型 + SUBLOT”；本系统不回写 MES，同一候选会持续存在到人工搬运并扫码过站，MesIngest 仍照常投影，但调度不得创建或恢复业务任务、不得派车。普通取消仍使用 `CANCELLED_BY_OPERATOR`；同一 SUBLOT 以后命中其它任务类型时属于不同业务键 |
| AREA 解析 | AREA 解析 = 两表（站点主数据 + AREA 归属）+ 命名规则自动派生；少数用显式覆盖 |
| 上线基线时间 | 上线基线时间 2026-08-01 |
| 合并查询性能 | 六类任务合并 SQL 性能实测已通过（约 3 秒级） |
| 车载任务与行程可见性 | 车载端不拥有任务或调度事实，但必须展示当前停靠作业清单、后续业务停靠计划和四维车辆概览；断线旧快照只读并标记过期 |
...
| 当前停靠作业清单 | 车载端显示的本站完整待装、待卸及复合作业清单；内容由服务端权威下发 |
| 后续停靠计划 | 服务端当前排定的后续业务站点顺序及作业目的；不包含调度地图路径节点 |
| 车载车辆概览 | 车载端并列显示运行、通信与业务就绪、当前作业、安全与联锁四个状态维度 |

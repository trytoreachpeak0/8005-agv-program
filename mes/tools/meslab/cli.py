...
def _parameters(value: str | None) -> dict[str, Any]:
    ...
    if value.startswith("@"):
        ...
        text = (
            possible_path.read_text(encoding="utf-8")
...
def _approval(value: str | None) -> dict[str, Any] | None:
    if not value:
        return None
    data = _parameters(value)
    allowed = {
        "status",
        "reference",
        "approved_by",
    ...
    if unexpected:
        raise ValueError(f"批准信息含未允许字段: {', '.join(unexpected)}")
    if data.get("status") != "approved":
        raise ValueError("批准信息 status 必须为 approved")
    if not data.get("reference"):
        raise ValueError("批准信息必须包含非空 reference")
    return data
...
def _parser() -> argparse.ArgumentParser:
    ...
    run.add_argument(
        "--approval-json",
        help="客户批准 JSON/文件；仅允许批准引用、人员、时间、范围等非敏感字段",
    )
    ...
    try:
        ...
        if args.command == "run-factory":
            ...
            run_dir = run_factory(
                ...
                mode=mode_name,
                approval=_approval(args.approval_json),
            )
        ...
        if getattr(args, "command", None) == "run-factory":
            print(f"错误：工厂执行失败（{type(exc).__name__}）", file=sys.stderr)
        else:

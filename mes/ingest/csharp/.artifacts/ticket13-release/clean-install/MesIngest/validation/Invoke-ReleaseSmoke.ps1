...
try {
    ...
    $host = [Diagnostics.Process]::Start($hostInfo)
    if ($null -eq $host) { throw 'Packaged Host did not start.' }

    $deadline = [DateTimeOffset]::UtcNow.AddSeconds($StartupTimeoutSeconds)
    $contract = $null
    do {
        if ($host.HasExited) { throw "Packaged Host exited during startup with code $($host.ExitCode)." }
        try {
    ...
    if (-not $SkipWatch) {
        ...
        $watch = [Diagnostics.Process]::Start($watchInfo)
        if ($null -eq $watch) { throw 'Packaged Watch did not start.' }
        do {
            if ($watch.HasExited) { throw "Packaged Watch exited during startup with code $($watch.ExitCode)." }
            $watch.Refresh()
            if ($watch.MainWindowHandle -ne [IntPtr]::Zero -and $watch.Responding) { break }
            Start-Sleep -Milliseconds 200
        } while ($watchStartedAt.Elapsed.TotalSeconds -lt $StartupTimeoutSeconds)
        if ($watch.MainWindowHandle -eq [IntPtr]::Zero -or -not $watch.Responding) {
        ...
        $window = [Windows.Automation.AutomationElement]::FromHandle($watch.MainWindowHandle)
        $requiredNames = @(
            '概览健康结论',
            'Host 接入状态',
            '最近轮询健康',
    ...
    foreach ($process in @($watch, $host)) {
        if ($null -ne $process) {
            if (-not $process.HasExited) {
                $process.Kill()
                $process.WaitForExit(5000) | Out-Null
            }
            $name = if ($process -eq $watch) { 'watch' } else { 'host' }

...
建议使用可配置映射，而不是依赖 AREA 与站点名称完全相同：
...
- 映射缺失时不派车，并产生可操作的配置异常。
- 映射变更应保留审计记录，便于追查机台移动和任务导航问题。
- AREA 到 AGV 站点映射由项目实施人员和现场系统管理员维护。

## 11. 已确认系统边界

- MES 只作为运输需求的查询来源。
- MES 数据库为 Oracle；连接信息保存在独立的 `mes/Oracle数据库连接信息.txt` 中，该文件含明文凭据，不得提交到公共仓库。
- 当前 MES 账号具有写权限，但 AGV 应用严禁写入 MES。数据访问层只能执行预置的合并 SELECT，并使用 Oracle 只读事务。
- 禁止提供通用 SQL 执行入口；任何非 SELECT 操作必须拒绝并产生安全报警，应用日志不得输出数据库密码。
- AGV 系统不需要理解 MES 内部 `入库`、`入站`、`完工` 的业务状态机。
- AGV 系统不向 MES 回写运输任务状态。
- 运输任务全生命周期保存在本系统。
...
11. 支持多个 AREA 映射到同一 AGV 站点，映射由项目实施人员和现场系统管理员维护。
...
16. 所有 AGV 任务状态保存在本系统，不依赖 MES 回写闭环。
...
23. MES 账号虽有写权限，应用仍必须使用只读事务且只能执行预置 SELECT，任何写操作属于高危违规。

...
- 当前 IT 是否能够提供花篮数量尚未确定。

### 2.3 EQP、AREA 与 AGV站点

- `EQP` 是 MES 机台编号。业务上 EQP 必须存在，为空时现场无法正常生产。
- `AREA` 是 MES 中记录的机台区域号，允许为空。
- 正常情况下，一个 EQP 只有一个有效 AREA。
- 同一 EQP 查询出不同 AREA 属于 MES 数据异常，系统必须上报，不能自动任选一个。
- AGV 实际导航目标是地图中的站点，不直接使用 MES AREA 执行导航。
- 系统维护 `AREA → AGV站点` 映射。
- 多个 AREA 可以映射到同一个 AGV 站点，因为多个紧密放置的机台可以共用一个停车或接驳点。
- 机台位置调整后，应通过配置更新 AREA 与 AGV 站点的映射，不应把映射逻辑硬编码在调度程序中。
...
4. 当前已定义的 MES 任务查询链路只读；本系统向 MES 回写的方向已纳入系统边界，但具体内容和接口尚未确定。
...
- 正式合并SQL位于`mes/sql/MES五类任务合并查询.sql`，执行步骤和测试内容位于`mes/sql/MES五类任务合并查询测试计划.md`。
...
2. 员工明确执行“新增一篮”，或在界面明确提示后再次扫描同一 SUBLOT。
3. 系统为该篮分配下一个可用仓位并开门。
4. 每次成功关门后生成递增的花篮序号。
5. 员工点击“该 SUBLOT 装载完成”，系统结束该任务本次装载。

不能仅凭短时间内重复读到相同条码就直接认定存在第二篮，否则无法区分真实多篮和扫码枪抖动、员工误扫。建议加入扫码防抖、开门前确认、仓门状态校验和装载完成确认。

装货和卸货均由系统自动打开对应仓门，用户关闭仓门即确认该仓位本次操作完成。系统应结合当前任务阶段区分“装货关门”和“卸货关门”，并更新仓位占用状态。

## 10. AREA到AGV站点映射

系统使用显式、版本化的可配置映射，不依赖 AREA 与站点名称完全相同。映射由 [[uc-024-maintain-area-station-mapping|UC-024]] 管理，并遵守 [[br-003-area-station-mapping|BR-003]]：
...
- 一个有效 AREA 在同一时间只能映射到一个有效 AGV 站点。
- 多个 AREA 可以映射到同一站点。
- 可选站点必须从 RCS/RIOT 当前地图站点列表同步，不允许手工录入未经 RCS/RIOT 确认存在的站点。
- 固定区域应配置独立站点代码：焊线/键合派工待送区、烘箱间、关卡区、三光区。实际地图站点编号通过配置指定，不在程序中写死。
- AREA 为空、映射缺失/停用/异常或目标站点失效时不派车，并产生可操作的位置/配置异常。
- 映射修改时关闭原有效版本并创建新版本；停用不物理删除，历史版本和任务引用必须保留。
- 映射变更只影响生效后创建的新任务。已有任务继续使用创建时冻结的 AREA、EQP、起终点和映射版本。
- 映射变更必须二次认证并记录操作人、修改前后值、原因、时间和结果。
- AREA 到 AGV 站点映射由 R-12 IT/软件维护人员和 R-13 AGV 运维/调度管理员维护。
...
- MES 回写仍按本文件既有结论保持 TBD；流程模板和步骤实例不新增、推断或绕过任何 MES 写能力，也不改变本地业务任务状态机独立运行的结论。

## 12. 已确认系统边界

- MES 是运输需求的查询来源，同时也是本系统未来回写数据的目标；当前只明确了查询流程，回写细节仍待确认。
...
- 本系统向 MES 回写的交互方向已确认纳入当前系统边界，但回写业务内容、字段、接口形式、触发时机、幂等、失败重试和补偿方式均为 TBD；这些事项评审确认前不得实现任何 MES 写操作。
- 运输任务全生命周期保存在本系统；后续 MES 回写属于状态或结果同步，不得成为本地任务状态机正常运行的前置依赖。
...
1. IT 是否提供每个 SUBLOT 的花篮数量、花篮标识或其他装载单位信息。
2. IT 不提供数量时，重复扫码与“新增一篮”按钮的最终现场交互方式。
3. 各车型每个仓位的尺寸、载重和兼容花篮类型如何配置。
4. 无车辆响应 1～3 分钟的最终默认阈值，以及除第 5 类最高优先级外其他任务的详细排序权重。
5. 合并SQL实测是否满足连续10轮全部成功、平均不超过5秒且最大不超过10秒的性能指标。
6. AREA 映射未来是否需要增加审批和版本回滚机制；当前不设审批流，采用二次认证、版本化生效和完整审计。
7. 固定区域在 AGV 地图中的实际站点编号。
8. 调度算法和复合停靠策略的完整方案，包括允许顺路停靠造成的最大配送延迟。
9. MES 回写的业务内容、字段、接口形式、触发时机、幂等键、失败重试、补偿方式、账号权限及验收标准。
...
7. AREA 为空或映射缺失时不得下发 AGV。
8. 同一 EQP 出现不同 AREA 时必须上报异常。
9. 同一 SUBLOT 同时命中多个任务类型属于阻断性异常；关卡与三光互斥。
10. 已取消任务重新出现时转人工确认，但不得人工创建新的正式MES任务。
11. 支持多个 AREA 映射到同一 AGV 站点；站点必须从 RCS/RIOT 同步，映射由 R-12/R-13 版本化维护，修改只影响新任务。
...
16. 所有 AGV 任务状态保存在本系统；后续 MES 回写仅用于同步已确认的数据，不得成为本地任务状态机运行所依赖的闭环。
...
23. MES 任务查询链路必须使用只读事务且只能执行预置 SELECT。回写接口和权限方案确认前，任何 MES 写操作均属于高危违规；后续回写不得复用通用 SQL 入口，必须采用独立、受控且可审计的接口或数据访问能力。
24. 每类最后正常非0数量、连续恢复次数和异常骤降保护状态必须持久化；重启后的第一次查询只建立恢复基线，第二次查询结合持久化基线判断进入或解除保护。
25. 正式生产任务只能来自MES；Mock任务使用独立来源和ID，不参与MES任务幂等与对账。
26. 合并SQL及测试计划分别保存在`mes/sql/MES五类任务合并查询.sql`和`mes/sql/MES五类任务合并查询测试计划.md`。
...
31. 流程异常处置只能按 UC-028 和 BR-006 执行有限重试、受限跳过或安全终止；UC-029 仅提供只读进度，不得借流程能力改变 MES 回写 TBD 或既有业务状态结论。

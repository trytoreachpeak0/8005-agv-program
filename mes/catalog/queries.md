...
| --- | --- | --- | --- | --- | --- | --- |
| `MES_TASK_UNION` | [SQL](../queries/mes-task-union/query.sql) · [元数据](../queries/mes-task-union/query.toml) · [契约](../queries/mes-task-union/README.md) | 无 | `TASK_TYPE`, `SUBLOT`, `AREA`, `EQP`, `STEP`, `DATES`, `PACKAGE` | [2026-07-16 五类原始查询](../sources/customer/2026-07-16/mes-task-original-queries/README.md) | 尚未迁入 `evidence/` | 待迁入/脱敏 |
...
| `MES_SCHEMA_INTROSPECTION` | [SQL](../queries/schema-introspection/query.sql) · [元数据](../queries/schema-introspection/query.toml) · [契约](../queries/schema-introspection/README.md) | 无 | 单一字段字典结果集，含 `FOUND` | 当前仓库内部为核验五类任务字段而编制，无独立客户 SQL 原稿 | 尚未迁入 `evidence/` | 不适用（元数据核验） |

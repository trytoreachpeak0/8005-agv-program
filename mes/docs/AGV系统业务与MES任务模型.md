...
7. MES 消失取消的保护与装载占用边界如下（取代“仅以第一个仓门打开为永久边界”）：
   ...
   - **进入运送后写死**：用户点击「开始运送」后，本趟已装的一个或多个 SUBLOT 进入运送；此后禁用上述清占用方式，开仓只按运送/卸货或异常处理，不得再靠开仓关门清零占用从而变回“可因 MES 消失取消”。本趟有效占用为 0 时「开始运送」按钮灰掉不可点。
...
5. 本趟存在至少一个有效占用时，员工可点击「开始运送」，使本趟已装的一个或多个 SUBLOT 进入运送；占用为 0 时该按钮灰掉不可点。

...
- 需求沟通中已经确认的物理运输流程、幂等规则、人工搬运竞争、多花篮规则，以及第一版一车可承载多个 SUBLOT 与取货区域类型卡控。
...
## 1.1 实现分期：MesIngest 与调度（2026-07-24）

本文后续章节仍描述**目标系统**（含派车、装载、运送、人工抑制等）。实现上按 [ADR-mes-0006](../../docs/adr/mes/0006-mes-ingest-vs-dispatch.md) 拆成两期，避免状态机与调度耦合阻塞 MES 获取：

| | **MesIngest（第一期）** | **调度系统（后续）** |
|---|---|---|
| 职责 | 轮询 `MES_TASK_UNION`、对账、重启恢复快照、投影 TransportDemand | 派车、优先级、仓位、装载/运送、流程模板 |
| 状态 | `VISIBLE` / `GONE`（加告警与 `PAUSED_ZERO_DROP`） | `PENDING`、派车、装载、运送等厚状态机 |
| 标识 | `demand_id` 为主键；`TASK_TYPE+SUBLOT` 仅对账与 VISIBLE 唯一约束 | 以 `demand_id` 挂接自有属性 |
| 接口 | 只读 API + 盯盘页 | 消费接入投影；人工永久抑制放在调度侧 |
| 不做 | 派车、装载、运送、永久抑制集合 | 不重做 MES 轮询对账 |
| 技术（第一期） | 见 [ADR-mes-0007](../../docs/adr/mes/0007-mes-ingest-tech-stack.md)：Windows Service + WPF + SQL Server；Oracle 默认 Thin（可切 Thick） | 后续另定 |

字段首次进入 `VISIBLE` 时冻结；快照漂移只告警不覆盖。正常现场流程中，同一 `TASK_TYPE+SUBLOT` 候选会持续出现，直到人工搬运并在 MES 扫码过站后消失。若同键在 `GONE` 后再现，MesIngest 按现有实现保留旧 GONE 实例、分配新的本地 `demand_id` 并产生对账告警；这只是接入投影的新实例，不表示 MES 形成了新需求，MesIngest 也不负责阻断。阅读第 6 节及之后的状态机时，须区分「接入已交付的投影语义」与「调度尚未实现的行为」；调度消费投影时另行执行永久抑制和可派判断。
...
- 操作员输入 SUBLOT 后，IT 通过 [`QUERY_ID: SUBLOT_BOX_COUNT`](../queries/sublot-box-count/) 查询 `max_box_count`（最大料盒数）；结合任务冻结的 `PACKAGE` 与 [`../reference/package-basket-capacity.md`](../reference/package-basket-capacity.md) 换算 V1 权威花篮数（见 9.2）。
- V1 花篮数不可由操作员或车载端修改；查询失败、`PACKAGE` 无唯一容量对照或无法换算时，本次装载报错，不分配仓位、不打开仓门，也不使用现场追加数量兜底（见 9.3）。
...
- MES 来源：2026-07-24 客户新增第 6 组查询（`WIRE_TO_NITROGEN`）。
- 触发含义：WireBond1（MES step=`焊线`）完工后，SUBLOT 处于 `焊线2`+`入库`，需送往氮气柜暂存；待机台空闲后再上 WireBond2。
- 起点：查询结果中 EQP/AREA 映射到的焊线机台 AGV 站点（规则同 `WIRE_TO_GATE`）。
- 终点：固定的氮气柜站点（现场站点编码待现场补录；本阶段查询契约不依赖具体站码）。
- 不含键合：仅焊线工艺有焊线1/焊线2 分道。
- PDA 扫码入柜后，该行从 MES 快照消失；之后再派往焊线2 由既有 `STAGING_TO_WIRE` 覆盖，不另建任务类型。
- 查询结果 AREA 表示焊线1完工机台的位置。
...
| --- | --- | --- |
| `TASK_TYPE` | 应用固定字符串/枚举 | 6组SQL通过`UNION ALL`合并时增加的任务类型 |
...
6. MES 数据库为 Oracle。6 组查询应通过 `UNION ALL` 合并成一条 SQL，各分支增加固定 `TASK_TYPE`，统一输出 `TASK_TYPE、SUBLOT、AREA、EQP、STEP、DATES、PACKAGE`。
...
7. MES 消失取消的保护与装载占用边界如下（取代“仅以第一个仓门打开为永久边界”）：
   ...
   - **进入运送后写死**：用户点击「开始运送」后，本趟已分配且已有有效占用的运输任务（可包含多个 SUBLOT）进入运送；此后禁用上述清占用方式，开仓只按运送/卸货或异常处理，不得再靠开仓关门清零占用从而变回“可因 MES 消失取消”。本趟有效占用为 0 时「开始运送」按钮灰掉不可点。
   - **保护期消失计数仍累加**；解除保护且占用归零后，若计数已 ≥2 可立即取消，不必重新从 0 数两轮。
8. 人工取消与同键抑制（情况 A：不装料直接取消）：
   - 允许条件：该任务无开仓未关会话、有效装载占用为 0、且尚未点击「开始运送」。
   - 人工取消针对幂等键 `任务类型 + SUBLOT`：调度将该键持久记入抑制集合，**永久**不再创建或恢复该键对应的正式业务任务；**不提供**解除/恢复入口。MesIngest 不读取该集合。
   - 同一 `SUBLOT` 若随后以**其他任务类型**出现在 MES 快照中，视为新幂等键；MesIngest 正常投影，调度不受原键抑制影响。
   - MES 查询中被调度抑制的键仍反复出现时，MesIngest 仍照常对账和投影；调度不得据此创建或恢复业务任务。
   - 因 MES 记录消失而转为 GONE（第 6 条）的投影：若同键之后再次出现，MesIngest 保留旧实例、分配新的本地 `demand_id` 并告警，不在接入层阻断；调度是否接受新投影按自身业务状态判断。仍不提供人工创建正式 MES 任务的入口，人工取消与 MES 消失须在调度原因中区分。
...
- `MANUAL`：调度将该 `任务类型 + SUBLOT` 写入**永久抑制集合**，不提供解除入口；MesIngest 仍照常投影，调度不得以新旧 `demand_id` 绕过抑制创建或恢复业务任务。
- `MES_DISAPPEARED`：**不**写入调度永久抑制集合；同键之后再次出现时由 MesIngest 分配新的本地投影实例并告警，调度如何消费该实例仍按自身业务状态判断。
...
调度消费 MesIngest 投影时：命中永久抑制集合的 `任务类型 + SUBLOT` 直接跳过，不创建或恢复业务任务、不派车；MesIngest 不读取该集合，仍照常对账与投影。
...
- 第一版一辆 AGV 可以同时承载多个运输任务（多个 SUBLOT）；每个运输任务仍以自身 `任务类型 + SUBLOT` 作为对账键，并保留独立的任务、装载和取消边界。
- 同一运输任务若对应多个花篮，仍按第 9 节在本任务内形成多条装载明细；多花篮与一车多 SUBLOT 是两个不同维度。
- 多个运输任务只有在已进入本次派车任务范围、仓位容量足够且各自取货合法时才能装入同一车辆；不能因为车辆仍有空仓位就临时装入范围外任务。
- 复合停靠、停靠顺序和顺路取货仍须遵守 [[../../requirement-documents/02-business-rules/br-001-dispatch-task-range|BR-001]]，不得改变任何运输任务冻结的业务起点类型与终点。
...
- 当前阶段车辆只装本次派车任务范围内已分配的运输任务；同类型机台存在其它任务，也不得据此绕过范围校验顺带装入。
...
#### 尚未由“一车多 SUBLOT”自动确定的能力

- 一车多 SUBLOT 本身不等于任意混装：不同任务类型能否组合、途中停靠如何增删或换序、顺路取货允许造成多大配送延迟，仍由派车范围、路线和任务类型规则分别约束。
...
- 当前阶段：一个 AGV 行程可以关联多个运输任务；调度器必须保留每个任务独立的起终点、状态和审计，不得把多个任务压成一个不可区分的任务记录。
...
花篮本身没有可验证身份，随机装入顺序也不构成领域身份。系统分层记录运输任务与仓位占用：

```text
运输任务唯一键 = 任务类型 + SUBLOT
活动仓位占用 = 运输任务ID + AGV ID + 仓位ID
```
...
系统不追踪实体花篮条码，也不生成 `basket_sequence`。追溯精度为“SUBLOT + AGV 仓位 + 占用时间段”；仓位占用记录的技术主键不代表花篮身份。

### 9.2 V1 换算并冻结权威花篮数

1. 操作员通过扫码或键盘输入 SUBLOT，系统定位唯一有效、且属于本次派车范围的待取货运输任务。
2. 系统按该 `SUBLOT` 执行 [`QUERY_ID: SUBLOT_BOX_COUNT`](../queries/sublot-box-count/)，得到正整数 `max_box_count`。
3. 系统用任务创建时冻结的 `PACKAGE` 唯一匹配 [`../reference/package-basket-capacity.md`](../reference/package-basket-capacity.md)，计算 `ExpectedBasketCount = ceil(max_box_count / capacity)`。
4. V1 将 `ExpectedBasketCount` 作为权威且不可修改的花篮数量；它不是估算、最低值或可由现场覆盖的参考值。
5. 系统一次分配数量相等的完整空闲仓位集合，并一次发起该集合的批量开锁。花篮没有身份，可以随机放入任一目标仓位；每个仓位最多一篮。
6. 全部目标仓位均达到有物、关门、锁闭和开锁输出复位后，完成该 SUBLOT 的装载闭环。

### 9.3 V1 计算失败与未来版本边界

料盒查询失败、无结果、空值、零/负数、超时，或者冻结 `PACKAGE` 缺失、容量未匹配、匹配冲突、无法换算时，本次装载报错并停止。服务端不得分配仓位或发送开锁指令，也不得给出默认数量、允许人工改数或通过重复扫码追加花篮；已有运输任务本身不因这次查询失败而自动取消。

未来版本可以另行评估把查询结果作为最低花篮数量或可调整参考值，但该行为不属于 V1，必须经过新的需求和架构决策后才能引入。

装货和卸货均由系统自动打开对应仓门；用户关闭仓门并结合传感器确认该仓位本次操作完成。系统应结合当前任务阶段区分“装货关门”（可生成或清零占用）和“运送/卸货关门”，并更新仓位占用状态。

### 9.4 操作员身份核验（非运输任务查询）

AGV 到站后，操作员扫描工牌一维码或手动输入工号，系统通过 MES 只读查询核验人员身份并建立操作会话（UC-043）。本查询与六类任务轮询无关。
...
- 五个固定区域（焊线/键合派工待送区、烘箱间、关卡区、三光区、氮气柜）各自配置一个固定 `station_name`，通过配置文件或数据库维护，可动态修改，程序不写死实际地图值；修改只影响后续创建的任务。
...
- 当前用于任务查询的 MES 账号虽然具有写权限，但查询数据访问层仍只能执行预置的合并 SELECT 及文档约定的其他只读 SELECT（六类任务合并查询、SUBLOT 料盒数查询、OP 操作员身份查询），并使用 Oracle 只读事务；不得利用该查询链路直接执行回写。
...
3. **顺路取货允许的最大配送延迟**  
   第一版确认一车可承载多个 SUBLOT（见 8.1）。多任务组合、停靠顺序和顺路取货须遵守 BR-001；尚未确定的延迟阈值和组合算法不得从“一车多 SUBLOT”自行推导。
4. **仓位规格配置**  
   各车型仓位数量、尺寸、载重和兼容花篮类型；当前车型暂定 8 仓，程序不得写死。
5. **AREA 映射治理增强**  
   未来是否增加审批流与版本回滚；当前方案为：不设审批流，采用二次认证、版本化生效和完整审计。
6. **封装对照表覆盖范围**
   [`../reference/package-basket-capacity.md`](../reference/package-basket-capacity.md) 是否覆盖全部 `PACKAGE` 取值；V1 未覆盖时按 9.3 报错并阻止开门，不得使用默认值。活跃样本不能称为全集，覆盖率与全集发现分别见 [`active-package-coverage`](../experiments/definitions/active-package-coverage/plan.md) 和 [`package-universe-discovery`](../experiments/definitions/package-universe-discovery/plan.md)。
7. **五个固定区域的具体 `station_name` 配置值**
...
3. **V1 花篮数量与开门方式**（替代原“花篮现场兜底交互”结论）
   2026-07-31 决策：操作员输入 SUBLOT 后，以 `SUBLOT_BOX_COUNT`、冻结 `PACKAGE` 和批准的容量对照换算权威 `ExpectedBasketCount`；该数量不可修改。无法计算时本次装载报错且不开门；计算成功时一次分配并批量打开等量仓位。重复扫码追加数量、默认数量和人工改数均不属于 V1。
   未来可以另行评估把查询结果作为最低花篮数或参考值，但不能据此改变 V1。有效装载仍以目标仓位有物关门为物理证据；「开始运送」、MES 消失保护和人工取消规则不因本决策自动改变。
4. **固定区域站点的配置方式**（原待确认第 4 项）
...
4. 将 MES 查询视为会重复、会消失的当前状态快照；使用带 `TASK_TYPE` 的 `UNION ALL` 合并 SQL 获取 Oracle 同一语句级快照（六类分支）。
...
10. 人工取消由调度针对 `任务类型 + SUBLOT` 永久抑制该键（无解除入口）；MesIngest 仍照常对账和投影，调度不得以新的 `demand_id` 绕过抑制。同一 SUBLOT 的其他任务类型仍正常处理；GONE 后同键再现由 MesIngest 分配新的本地投影实例并告警。不得人工创建新的正式 MES 任务。
...
12. 运输任务、仓位占用记录和 AGV 行程必须分层建模；不追踪实体花篮，也不以装入顺序生成花篮身份。
13. 第一版一车可承载多个 SUBLOT；每个运输任务保持独立身份、起终点、状态与审计。取货按各任务起点区域类型校验，不得跨装片、焊线/键合、派工待送区三类起点，也不得装入本次派车任务范围之外的运输任务。

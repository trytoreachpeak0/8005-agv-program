...
- 人工取消（无开仓会话、无有效占用、未开始运送）：调度对幂等键 `TASK_TYPE + SUBLOT` 永久抑制，不自动恢复业务任务，不提供解除入口；同一 SUBLOT 的其他 `TASK_TYPE` 仍正常处理。MES 仍可见该键时 MesIngest 照常投影，调度不得建单或派车。
- 因 MES 消失转为 GONE 后同键再次出现时，MesIngest 保留旧实例、分配新的本地 `demand_id` 并告警，不在接入层阻断；调度如何消费新投影按自身业务状态判断。
...
- 一个 SUBLOT 可对应一个或多个花篮；每篮占用一个仓位，系统记录 SUBLOT 与仓位的占用关系。
- AGV 可混装不同任务类型并多站取送，但每次取货必须校验任务起点。
- 仓位数量和规格按车型配置；当前车型暂定 8 仓，程序不得写死。
- 不追踪实体花篮条码，也不生成花篮序号；只记录 SUBLOT、AGV 仓位和占用时间段。
- 操作员输入 SUBLOT 后，以 `SUBLOT_BOX_COUNT` 查询结果、任务冻结的 `PACKAGE` 和批准的容量对照换算 V1 权威 `ExpectedBasketCount`；该数量不可由操作员或车载端修改。
- 查询或换算失败时，本次装载报错，不分配仓位、不开门；成功时一次分配并批量打开等量仓位。有效装载仍以目标仓位有物关门为准；以「开始运送」进入运送。
...
- 目的：按现场输入的 SUBLOT 查询各工序料盒计数的最大值，结合冻结 `PACKAGE` 的容量换算 V1 权威花篮数量。
- 正式查询：[`../queries/sublot-box-count/`](../queries/sublot-box-count/)
- 目录登记：[`../catalog/queries.md`](../catalog/queries.md)
- 输入契约：绑定参数 `sublot`，禁止字符串拼接。
- 输出契约：单值 `MAX_BOX_COUNT`；无结果、空值、非正数、失败或超时不得取消运输任务，但必须阻止本次仓位分配和开门。
...
4. V1 花篮数量由 `SUBLOT_BOX_COUNT` 与冻结 `PACKAGE` 容量换算，结果不可修改；计算失败时本次装载报错且不开门，成功时批量打开等量仓位。

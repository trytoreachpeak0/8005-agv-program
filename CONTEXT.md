...
地图不一致、站点缺失、全部路线不可达或必要准入配置缺失等使 TransportDemand 不存在任何潜在合法车辆的状态；它必须立即形成按任务与原因去重的告警并持续更新，不等待防饥饿阈值。车辆只是忙碌、容量暂满或单车位站点暂被占用属于 UnassignedDemandBacklog，达到分区批准阈值后才升级告警。
_Avoid_: 暂时没空车即结构性阻断、结构性无解仍等待老化阈值、每轮重复新建相同告警、告警后绕过门禁
...
把已选 TransportDemand 或复合任务集合分配给某台合格车辆后，相对该车分配前既有计划增加的预计行程代价；满足途中追加门禁的在途车辆与空闲车辆均以此参加同一选车比较，不因“在途”或“空闲”身份自动优先。
_Avoid_: 车辆到最近任务的直线距离、空闲车固定优先、在途车固定优先、覆盖 EnRoutePickupDeliveryDelay

**RouteCostEvidenceBoundary（路径成本证据边界）**:
无法确认车辆到下一站可达时该车退出本轮候选；可达性已经确认但 VehicleMarginalRouteCost 缺失、过期或不可比较时，保留该车并跳过路径成本排序层、记录原因后继续比较后续层。任何直线距离或弱替代值都不得冒充 RIoT RouteCost，路径证据缺失只作为少见异常边界而非常态降级路径。
_Avoid_: 可达性未知仍派车、直线距离证明可达、排序成本缺失即整轮停派、弱替代冒充 RIoT 路径成本
...
8005 唯一获准自动建单的 RIoT 调用层级，普通搬运、前往充电桩和其它已批准单段移动意图都只允许以稳定 upperId、已核验车辆、地图和目标站点通过 `byDefaultMissions` 创建，并须先确认车辆可调度、RouteCost 可达且未占用 RIoT 车辆订单名额。充电建单不设专属重试次数；结果未确认时保留车辆、充电意图、目标桩和预占的原绑定，统一按 RIoTRetryReconciliation 先对账后重试，不得换号、重新选桩、重复建单、提前释放预占或表示已经前往/开始充电。
...
**充电桩预占（ChargingStationReservation）**:
8005 为一辆尚未到桩的 AGV 原子取得的单车独占充电桩使用权；取得后不因其它等待车辆电量更低而被抢占。
...
若要推翻就必须重做既有实现的性质。完整产品语境下已识别五条：车辆占用必由一条 TransportDemand
引起、全系统至多一辆车与一条未收敛 journey、一次承诺等于一条 Demand 加固定两段行程、站点没有
独占概念、车载对 Demand 是只读投影绝不发现选择或绑定。它描述现状，不是需求，也不是设计目标。

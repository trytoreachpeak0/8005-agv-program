...
本次物理操作中已经发出开锁命令或发送结果不确定、尚未闭环的仓位集合。按一次一扇开锁（SingleDoorUnlockSequence），它至多包含一个仓位；取消授权到达时正在装货的那扇门留在集合里，直到它闭环。
_Avoid_: 整个指令仓位清单、尚未开始的仓位、仅收到成功反馈的仓位、多仓同时开锁集合
...
**一次一扇开锁（SingleDoorUnlockSequence）**:
车载上位机执行业务仓位操作的方式：同一台车任一时刻至多一个仓位处于已发出开锁、尚未闭环的状态。前一仓位达到目标态（装货与纠错重放为 OCCUPIED，卸货与各类清空为 EMPTY，并且锁闭、开锁输出已复位）后才打开下一仓位；同一指令的目标集合内先 FRONT 分组后 REAR 分组，组内按仓位号从小到大，在 8005 当前仓位模板下等于仓位号升序。前一仓位进入 UNKNOWN 时不再打开后续仓位，整个操作暂停转人工恢复。适用于装货、卸货、取消清空、补偿清空、纠错重放、故障货物受控取货、安全收尾与操作员手工重开；维护开门模式与强制机械取出例外。见 `REQ-0357`、ADR-cross-0061。
_Avoid_: 批量开锁、依次发脉冲后并发等待闭环、跳过 UNKNOWN 继续开、操作员任意顺序放取

**批量开锁（BatchUnlock）**:
已废止的执行方式（2026-09-18 起由一次一扇开锁取代，`CP-0004`、ADR-cross-0061）：车载上位机对一个目标仓位集合一次打开全部仓门，同一 IO 模块内用 Modbus `0x0F` 写多个线圈，跨模块按分组并行发送。只在阅读旧决策与旧证据时出现，业务仓位操作不得再使用。
_Avoid_: 把批量开锁当作现行规则、维护开门模式的同时开多扇（那是维护模式例外，不叫批量开锁）
...
**期待动作超时（SlotExpectedActionOverdue）**:
车载上位机执行装货或卸货仓位操作时，当前仓位自本次操作第一次对它发出开锁起累计等待操作员达到门槛（默认 3 个 OperationTimeout 提示节拍，自动重开不清零，车载端配置、投运实测标定）仍未闭环且未判为 UNKNOWN，于是经车载告警快照向服务端上报、并在 OnboardHmi 提示「已上报」的事实。它只让人看到，不改变执行器行为，不判失败、不进恢复，也不改变现场升级规程的责任次序。见 `REQ-0358`（`CP-0005`）、ADR-cross-0062。
_Avoid_: 把超时当成故障、车载端据时间自行判 UNKNOWN、按站点期限计时、重开后重新计时、推送通知

**人工判故障（SlotFaultDeclaration）**:
持 ExceptionRecoveryPermission 的管理员依据现场事实，在服务端看板上把已上报期待动作超时、仍在等待操作员、读数有效却无法闭环的当前仓位判为 UNKNOWN 的动作；须认证个人身份并填写故障类别与现场说明。服务端可靠下发后，车载端核对该仓仍在等待即中止执行器、不再开锁，报该仓 UNKNOWN、后续仓 NOT_STARTED，操作随即转 RecoveryRequired、旅程转 Blocked，再按异常处置会话收尾；仓位已闭环时车载端拒绝，判定撤销。判定不开门、不结算、不取消需求、不改 MES、不标记仓位不可操作。依赖协议 v3.0.0 新增的两条消息；之前以重启车载端程序、由中断结算报 UNKNOWN 过渡。见 `REQ-0359`（`CP-0005`）、ADR-cross-0062。
_Avoid_: 在 OnboardHmi 上判定、操作员或班组长判定、把人未放料当成故障、用维护开门模式代替、用判定取消卸货、仅服务端置阻断而不通知车载端
...
以稳定 IntegrationSliceId 标识、能够由共享协议向量和确定性故障脚本独立复现的一段控制服务端与车载端之间的跨端能力；它固定前置切片、输入、逐步输出、持久事实、禁止副作用和最终状态，并分别经过双模拟对端门禁及精确两端候选构建的真实联合运行。只有双端能力才立切片：服务端与 RIoT、MES 或人之间的单端能力不是切片，需要多个并发会话才能表达的行为（例如多车并发）在保形向量的单会话格式下也无法成为切片，两者的验收由 SemiPhysicalScenarioLayer 与 FieldAcceptanceWindow 承担——不立切片不等于不验，只是判据不在门禁模型里。IntegrationSliceId 属于某个 IntegrationSliceFamily；协议大版本换代时每个切片都要在新的 ProtocolReleaseIdentity 上重过门禁，编号只在切片范围与名实不变时沿用。切片完成不是产品发布批准，也不能替代工厂试运行。
_Avoid_: 团队任务编号、仓库开发阶段、一次“大联调”、只对 Fake 通过即完成、现场成功倒推通过、把单端工程算作切片

**IntegrationSliceFamily（切片家族）**:
某一个 ProtocolRelease 之下全部 IntegrationSlice 的完整编号集合，由 id 前缀、编号规则、切片总数、顺序与前置关系共同确定；家族随协议大版本整体换代而不是逐个增补：新大版本下全部切片都在新的 ProtocolReleaseIdentity 上重过门禁，前一代的门禁证据随其不可变 tag 原地冻结，此后只能被引用为“前一代的对应切片”，不能被复用为新一代的通过结论。编号前缀只在切片范围或名实改变时更换——`W2G-IS` 换成 `FP-IS`，是因为完整产品不再只是 WIRE_TO_GATE；范围与名实不变时沿用编号，两代证据由 ProtocolReleaseIdentity 区分。
_Avoid_: 以沿用编号代替证据重跑、范围或名实已变仍沿用编号、两套编号并存于同一 index、把批次或业务分簇编进 IntegrationSliceId、以家族更名代替证据重跑
...
生产调度从 MVP 部署一次性转到 v2 线的 FieldAcceptanceWindow。进入门槛是切换路径上全部 ImplementationBatch 的出口条件达成、在备用车上以独立 v2 实例完成的预演窗口通过（预演不含自动充电，因为唯一在用的充电桩由生产车辆占用）、MVP 修复同步的必需项合入、空载急停演练完成，以及等待点、固定站点绑定与充电桩名册等现场前置就绪。窗口内按四个阶段放开：一台车一趟只带一条需求，两台车，三台车，最后开启一车多需求；每个阶段在跑的每项能力都至少完整走通一次且无需人工处置后才进入下一阶段，出现安全类异常即停在当前阶段。仓位分侧、自动充电与空闲返回从第一阶段起同时首次上线，是「不在同一次窗口叠加多个未验证新能力」的明确例外：分侧无法单独关闭，自动充电不允许以人工充电替代。唯一回退是换回切换前的 MVP 部署与数据，不修改 MVP 代码。
...
服务端为一次多仓装货或卸货物理尝试生成的稳定唯一编号，贯穿指令接受、逐仓执行、结果补报、暂停续行和恢复对账；通信重发沿用原编号，终态后重新尝试使用新编号。
...
车辆离站前由已核验操作员针对一个 DemandId 发起的任务终止流程；未产生物理装载时可直接终结，已经部分或全部装载时，对该任务实时确认为 OCCUPIED 的目标仓位按一次一扇开锁逐个打开并全部清空，最终使该任务完整目标仓位范围均达到 EMPTY、由锁传感器确认锁闭且开锁输出回读确认为复位。其它 SUBLOT 的仓位、任务与确认记录不受影响；但同车另一 SUBLOT 尚处于物理装货或清空过程中时不得穿插本次取消，必须等待其完成或取消到稳定边界。服务端可靠接受 ALL_EMPTY 并发布新版作业清单后，释放仓位可以用于新的 SUBLOT。获得服务端取消授权时尚未锁闭的当前装货仓位可以直接取出并以 EMPTY 安全收尾，它闭环之前不得打开其它仓位；正式提交后发起取消属于保留原确认与提交事实的补偿，不是撤销或抹除历史。
...
**UnloadBatch（卸货批次）**:
一次到站时服务端识别出的本站全部可卸仓位组成的一次物理操作，可包含多个 Sublot，由一个仓位操作尝试编号关联目标集合，按一次一扇开锁逐个打开。其业务提交粒度受卸货 Sublot 完整性策略控制；8005 项目关闭该策略，因此逐仓达到物理完成条件后即可独立清空该仓位业务状态。
...
项目级统一决定卸货业务状态是否按 Sublot 整体提交的策略；8005 项目关闭，因此本站全部可卸仓位可以跨 Sublot 在一次卸货中逐个打开，并在每仓达到 `EMPTY + 锁闭 + 开锁输出已复位` 后独立清空业务状态。策略不能按站点或单次卸货临时切换。
_Avoid_: 操作员临时选择、一次卸货必然整批提交、装货完整性策略

**OperationSession（操作会话）**:
服务端为一次到站建立的现场操作上下文，关联 AGV、站点、项目策略和审计，可连续处理多个 Sublot。同一站有多条待装或待卸的 DemandId 时共用这一个会话，服务端逐条串行处理，任一时刻只有一条需求处于录入或仓位操作中。协议上，CurrentStopWorklistSnapshot 只在顶层带一个 operationSessionId，就是本站这一个；清单项不带会话号，车载端不从清单推断当前是哪条需求；仓位操作命令逐条带自己的 DemandId，会话号都填本站这一个。会话在 StopClosureCommit 时结束，无论是操作员结束本站还是 StationDepartureWaitTimeout 届满；持货等单期间结束本站不等于车辆离开。车辆下一次到站（包括回到同一站）是新会话；今天单需求旅程的取货与卸货两个停靠共用一个会话号，那是为逐字保留既有报文而沿用，「每次到站一个」约束以后新产生的停靠。需要身份核验时，每个 Sublot 在首次开锁前绑定当时已核验工号，同一 Sublot 内不得更换；一个 Sublot 经服务端确认完成后可以核验并切换工号。掉线和重启保留当前绑定，不重复验证。
_Avoid_: 管理端登录会话、车载登录、班次会话、每条需求一个 OperationSession、清单项级会话
...
异常处置人员在 ExceptionRecoverySession 中使用的受控开仓能力；一次身份验证后可重复开关本事件范围内的多个合格仓位，不逐仓审批或重复认证。合格仓位是服务端没有活动 Sublot、运输任务、其它货物业务绑定或未收敛仓位操作的物理仓位；其 SlotOccupancyState 为 OCCUPIED 或 EMPTY 都不影响进入范围，OCCUPIED 且无业务绑定时另形成 SlotStateMismatch。每个实际开启的仓位仍须完成锁闭和开锁输出复位，模式期间整车保持停站并阻止新移动和仓位作业，退出后重新完成 PreDepartureSafetyCheck。进入时整车不得有未收敛的业务仓位操作；存在未收敛操作时，无论原因是否为硬件故障，都须先按异常处置闭环收尾（`REQ-0226` 经 `CP-0004` 修订项三，ADR-cross-0061）。
_Avoid_: 审批入口、逐仓重复认证、跨异常开仓、开启仍绑定 Sublot 或任务的仓位、以光幕状态覆盖服务端业务绑定、维护中移动车辆、未重新安全核验即继续移动、业务操作未收敛时进入
...
证明车辆实际停止的正面事实：急停状态经回查确认为 `CAN_RECOVER` 或 `CAN_NOT_RECOVER` 时即视为已停稳，不再另读运动状态、速度或位置，急停状态回到 `OK` 后不再适用；除此之外须取得组合事实——新鲜的 `movementState` 明确为非移动状态、连续多次位置观测保持不变，且期间没有新的移动迹象；具体连续次数和时间窗口由统一监控策略规定。订单 `HELD` 或单次静止查询均不能单独构成停稳证明，任一必要事实未知时保持未证明。
_Avoid_: OrderHold 成功、急停调用被受理即停稳、单次速度或位置快照、状态未知时推定停稳、急停已锁住仍另读速度或位置

**远程急停失效处置（RemoteEmergencyStopFailureResponse）**:
远程 `triggerEmergency` 失败、超时或结果未知且仍无停稳证明时的持续保护状态：急停状态未确认前按受控退避持续重试并回查；确认 `CAN_RECOVER` 或 `CAN_NOT_RECOVER` 后停止重复触发，并按停稳证明视为已停稳，继续监控急停状态；原因消除前急停意外恢复 `OK` 时立即重触发，经人员确认后由服务端解除的不属于意外恢复。系统发出最高优先级现场告警，并在停稳得到证明或完成现场物理隔离前始终按车辆可能仍在移动处理；现场已有的物理急停可由任何现场人员立即操作且不要求系统登录或审批，断电、抱闸等专业隔离只由具备相应作业资质者执行。
_Avoid_: 调用失败即放弃、急停已确认仍盲目刷调用、急停调用被受理即视为停车、告警即视为停车、人员确认后的服务端解除当作意外恢复、等待审批后按物理急停、无资质断电或解抱闸
...
8005 只能对已进入 `CAN_RECOVER` 的软件急停调用 `cancelEmergency`，解除有两条路径。系统自动触发的急停在全部触发原因消除、仓门安全、具备停稳证明，并完成订单、位置、故障状态和未结调用对账后自动解除，同时存在多个原因时缺一不可。系统自动触发、车载端请求或服务端人员请求的 8005 急停，也可由服务端已登录且有车辆查看权限的人员对选中车辆确认原因已消除、车上无货且全部仓门已关闭并记下身份，再由服务端调用解除并结束本次急停，不重触发；车载端不能确认，不要求第二人确认。外部系统触发或来源不明的急停不自动解除，必须由人员明确请求；`CAN_NOT_RECOVER` 禁止软件解除，调用后必须回查 `emergencyState=OK`。人员确认路径要求该车没有未终结的订单，须先取消订单再解除。解除只结束急停，不恢复派车资格。
_Avoid_: CAN_NOT_RECOVER 强制解除、部分原因消除即解除、手动或未知来源自动解除、只看 RIoT 不看车载安全状态、调用成功即解除、车载端确认解除、人工确认要第二人、人员确认后的解除仍当作意外恢复重触发

**仓门不安全移动急停保持（DoorUnsafeMovementEmergencyLatch）**:
车辆在仓门未能证明安全锁闭时发生移动或失去停稳证明后，8005 自动触发并持续保持软件急停；外部系统提前解除时立即重触发并告警，车载端确认全部仓门安全锁闭且开锁输出复位后才自动退出保持，另可按软件急停解除的人员确认路径解除。已授权正常装卸只维持 StationOperationGuard，不告警或急停；授权外异常但已有停稳证明时阻断移动并高优先级告警，出现移动或停稳证明失效才进入本状态。

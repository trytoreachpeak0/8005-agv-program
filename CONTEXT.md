...
车载上位机执行装货或卸货仓位操作时，当前仓位自本次操作第一次对它发出开锁起累计等待操作员达到门槛（默认 3 个 OperationTimeout 提示节拍，自动重开不清零，车载端配置、投运实测标定）仍未闭环且未判为 UNKNOWN，于是经车载告警快照向服务端上报、并在 OnboardHmi 提示「已上报」的事实。它只让人看到，不改变执行器行为，不判失败、不进恢复，也不改变现场升级规程的责任次序。见 `REQ-0358`（`CP-0005`，草稿）、ADR-cross-0062。
_Avoid_: 把超时当成故障、车载端据时间自行判 UNKNOWN、按站点期限计时、重开后重新计时、推送通知

**人工判故障（SlotFaultDeclaration）**:
持 ExceptionRecoveryPermission 的管理员依据现场事实，在服务端看板上把已上报期待动作超时、仍在等待操作员、读数有效却无法闭环的当前仓位判为 UNKNOWN 的动作；须认证个人身份并填写故障类别与现场说明。服务端可靠下发后，车载端核对该仓仍在等待即中止执行器、不再开锁，报该仓 UNKNOWN、后续仓 NOT_STARTED，操作随即转 RecoveryRequired、旅程转 Blocked，再按异常处置会话收尾；仓位已闭环时车载端拒绝，判定撤销。判定不开门、不结算、不取消需求、不改 MES、不标记仓位不可操作。依赖协议 v3.0.0 新增的两条消息；之前以重启车载端程序、由中断结算报 UNKNOWN 过渡。见 `REQ-0359`（`CP-0005`，草稿）、ADR-cross-0062。

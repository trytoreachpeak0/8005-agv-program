...
模拟器通过统一仓位动作支持终点站取出流程。开锁由主系统写 Modbus DO 发起，正常情况下弹簧自动弹门；WPF 与控制 API 负责模拟 `关门`、`放料`、`取出`。主系统可以用 Modbus `0x0F` 对同一模块多个仓位批量开锁，再并行对已打开仓位执行“取出→关门”。
...
- WPF 与自动化控制 API 均暴露 `关门`、`放料`、`取出` 三个人工动作；开门由 Modbus 开锁成功后自动发生。
- 有效开锁脉冲自动使门打开、锁 DI 变为未锁；DO 复位不令锁 DI 自动锁闭。`关门` 在 DO 已复位时更新内部门状态并使锁 DI 锁闭，门状态不映射为独立 DI。
...
- **TC-FR-015-001（编号预留）**
  ...
  - **When** Modbus 开锁自动弹门后依次执行取出、关门
  - **Then** 动作均成功，取出后光幕 DI 表示无遮挡且派生占用状态为空闲
- **TC-FR-015-002（编号预留）**
  - **Given** WPF 和自动化控制 API 指向相同初始状态的仓位
  - **When** 分别通过两种入口执行关门、放料、取出
  - **Then** 三个动作的前置条件、状态变化和错误语义一致
- **TC-FR-015-003（编号预留）**

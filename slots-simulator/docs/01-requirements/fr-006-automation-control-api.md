...
提供一套独立于 Modbus 协议、默认仅监听 localhost 的 HTTP/JSON 控制 API，代表人工操作、测试前置构造或故障注入，供开发编写自动化测试脚本调用。API 不提供"开锁"动作；开锁指令只能由被测主系统通过 Modbus DO 下发。面向仓位的人工动作统一为"开门、关门、放料、取出"。
...
- 每个仓位的开门、关门、放料、取出动作均有对应的可编程调用方式；API 和 WPF 均不提供开锁动作，开锁只接受 Modbus DO。
...
- **TC-FR-006-001（待建立）**：Given 服务按默认配置启动；When 从本机调用 HTTP/JSON 开门接口并从非本机地址尝试连接；Then 本机请求可达，非本机请求不可达，且 API 清单中不存在开锁接口。

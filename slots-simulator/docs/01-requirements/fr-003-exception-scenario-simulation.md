...
- 开锁/弹门失败：下发开锁脉冲后 DO 正常复位，但仓门未弹开且锁状态 DI 保持“锁闭”；主系统据此识别开锁失败，不自动重复脉冲。
- 闩锁失败：控制 API 模拟关门后，锁状态 DI 仍保持“未锁”，表示门未能真正闩合；该故障与 DO 是否已经复位无关。
...
- **TC-FR-003-002（预留）**：Given 锁状态 DI 配置为延迟反馈；When 主系统写开锁 DO=1；Then 延迟期内 DI 保持锁闭，达到配置时长后门弹开且 DI 变为未锁，模拟器本身不产生业务超时结论。
...
[[dr-010-do-control-mode|DR-010]]（开锁输出与锁反馈独立）、[[dr-014-fault-injection-mirror-exception|DR-014]]（开锁/弹门失败、闩锁失败与光幕反馈故障）、[[fr-014-multi-io-module-simulation|FR-014]]（IO 通信异常按模块实例粒度触发）。
